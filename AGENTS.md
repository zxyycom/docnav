...
Docnav 是 CLI-first 的结构化文档导航系统。核心入口是 `docnav` CLI，所有调用入口共享同一契约，通过有限、可继续的流程读取大型文档：
...
- 输出分层：原始协议保证稳定校验；阅读输出保证信息密度。两类输出复用业务语义，但不复用传输包装。
- ref 由 adapter 生成和解析；`docnav` 和调用入口只原样传递。
...
- 涉及实现代码、重构、测试脚本、验证脚本或跨模块修改时，除对应主规范外，必须先读取 `docs/coding-style.md`；交付前按其中“变更前后自检”检查本次改动。
- CLI 命令优先选择只读、可复现、范围明确的命令；验证命令按改动范围选择，避免无关全量操作。
- 改动跨 Rust 行为、OpenSpec、schema、示例、输出边界或多个包边界时，最终优先运行 `bun run verify:docnav-workspace`；纯提示词或说明文档改动可用 `dnm outline`、局部 diff 等范围匹配的验证。
