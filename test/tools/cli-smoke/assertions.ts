...
export function expectExit(record: ExternalValue, expected: ExternalValue) {
  expect(record, record.exitCode === expected, `exit code is ${expected}`);
}

export function expectStdoutEmpty(record: ExternalValue) {
  expect(record, record.stdout.length === 0, "stdout is empty");
}

export function expectStderrEmpty(record: ExternalValue) {
  expect(record, record.stderr.length === 0, "stderr is empty");
}

export function expectStdoutIncludes(record: ExternalValue, value: ExternalValue) {
  expect(record, record.stdout.includes(value), `stdout includes ${JSON.stringify(value)}`);
}

export function expectStderrIncludes(record: ExternalValue, value: ExternalValue) {
  expect(record, record.stderr.includes(value), `stderr includes ${JSON.stringify(value)}`);
}

export function expectStdoutWarning(record: ExternalValue, expectedTokens: ExternalValue) {
...
export function expectStderrWarning(record: ExternalValue, expectedTokens: ExternalValue) {
...
export function parseJson(record: ExternalValue) {
  try {
...
export function parseReadableViewHeader(record: ExternalValue, output = record.stdout) {
...
export function expectNoProtocolEnvelope(record: ExternalValue, value: ExternalValue) {
  const found = findProtocolEnvelopeKeys(value);
  for (const key of protocolEnvelopeKeys) {
    const leakedPaths = found.filter((item: ExternalValue) => item.key === key).map((item: ExternalValue) => item.path);
...
export function expectProtocolSuccess(record: ExternalValue, value: ExternalValue, operation: ExternalValue) {
...
export function expectProtocolFailure(record: ExternalValue, value: ExternalValue, operation: ExternalValue, code: ExternalValue) {
  expect(record, value.protocol_version === "0.1", "protocol_version is 0.1");
  expect(record, typeof value.request_id === "string" && value.request_id.length > 0, "request_id is nonempty");
  expect(record, value.operation === operation, `operation is ${String(operation)}`);
...
export function expectNoWarningsField(record: ExternalValue, value: ExternalValue, label: ExternalValue) {
  expect(record, !Object.hasOwn(value, "warnings"), `${label} omits warnings`);
}

export function expectStructuredWarning(record: ExternalValue, warning: ExternalValue, expectedTokens: ExternalValue, label = "CLI argv") {
...
export function expectOutlineResultsEquivalent(record: ExternalValue, actual: ExternalValue, expected: ExternalValue, summary: ExternalValue) {
...
export function expectReadResultsEquivalent(record: ExternalValue, actual: ExternalValue, expected: ExternalValue, summary: ExternalValue) {
  for (const field of ["ref", "content", "content_type", "cost", "page"]) {
    expect(record, actual?.[field] === expected?.[field], `${summary}: ${field} matches`);
  }
}

export function expectReadableViewBlockRestoresField(record: ExternalValue, output: ExternalValue, pointer: ExternalValue, expectedPayload: ExternalValue) {
  const headerBoundary = output.indexOf("\n\n[block ");
...
export function expectReadableViewFieldValue(record: ExternalValue, output: ExternalValue, pointer: ExternalValue, expectedValue: ExternalValue) {
  const header = parseReadableViewHeader(record, output);
...
export function expectNoReadableViewBlocks(record: ExternalValue, output = record.stdout, label = "readable-view") {
  expect(record, !output.includes("\n\n[block "), `${label} has no block sections`);
  expect(record, !output.includes("\n[endblock "), `${label} has no end markers`);
}

export function expectFindResultsEquivalent(record: ExternalValue, actual: ExternalValue, expected: ExternalValue, summary: ExternalValue) {
  expect(record, actual?.page === expected?.page, `${summary}: page matches`);
  expect(
    record,
...
export function expectInfoResultsEquivalent(record: ExternalValue, actual: ExternalValue, expected: ExternalValue, summary: ExternalValue) {
...
export function expectNoJsonPayloadInStderr(record: ExternalValue) {
  const jsonLine = record.stderr
    .split(/\r?\n/)
    .map((line: ExternalValue) => line.trim())
    .find((line: ExternalValue) => looksLikeJson(line));
...
export function expectIncludes(record: ExternalValue, values: ExternalValue, value: ExternalValue, summary: ExternalValue) {
  expect(record, Array.isArray(values) && values.includes(value), summary);
}

export function expect(record: ExternalValue, condition: ExternalValue, summary: ExternalValue) {
...
export function assertSetup(condition: ExternalValue, message: ExternalValue) {
  if (!condition) {
    throw new Error(message);
  }
}

export function looksLikeJson(value: ExternalValue) {
  const trimmed = value.trim();
  return trimmed.startsWith("{") || trimmed.startsWith("[");
}

export function containsProtocolResponseEnvelope(value: ExternalValue) {
  return value.includes("\"protocol_version\"") && value.includes("\"ok\"");
}

function findProtocolEnvelopeKeys(value: ExternalValue, path = "$"): ExternalValue[] {
...
function jsonPointer(value: ExternalValue, pointer: ExternalValue) {
  if (pointer === "") {
    return value;
  }
  return pointer
    .split("/")
    .slice(1)
    .reduce((current: ExternalValue, segment: ExternalValue) => {
...
function deepEqual(actual: ExternalValue, expected: ExternalValue): boolean {
