...
- [ ] 已标出 touched boundary：parser/frontmatter、ref generation/parsing、path resolution、adapter invocation、stdio JSON、output mode、schema validation、generated docs/examples、dependency script 或 external command。
...
- [ ] Ref 在 owning adapter 外保持 opaque；`docnav` 和其他 adapter 不解析、不改写、不拼装 ref。
...
- [ ] Raw `protocol-json`、`readable-view` 与非文档 PlainText 没有混用 schema、wrapper、pagination envelope 或稳定性承诺。
- [ ] CLI input、adapter output、examples、fixtures 与 generated protocol material 都按对应 schema 或 contract 验证。
...
- [ ] 对跨 Rust、adapter、schema、examples 或 docs 的 security-sensitive change，feasible 时要求 repository workspace verifier，并记录无法运行时的原因。
