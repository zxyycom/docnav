...
只有在入口命令失败、需要 stop/batch/concurrent session/runtime setup，或维护 approval bridge 脚本时读取本文件。
...
dispatcher 默认 `acceptEdits`。如果 Claude 报告工具调用被阻塞，切换到 approval bridge。若仍适合 batch retry，使用最窄的 `-AllowedTools` pattern，而不是 `bypassPermissions`。

## 等待策略

- approval session 在后台运行；用有界 `status --wait-seconds` 轮询。
- CLI 记住最新 session；非并发场景可省略 `--session-directory`。
- Claude Code 可能很慢。调用 batch dispatcher 时，工具超时至少设为 `7200000` ms；短超时只能视为不确定结果。重试后仍超时，就拆小任务。
