...
- 若任务涉及 Docnav protocol、schema、examples、CLI、adapter 或 MCP mapping，结合本仓库 `docs/navigation.md` 的 role path 读取对应主规范。
- official source hierarchy 适用于外部技术决策；Docnav 自身 contract 以本仓库主规范、schema、examples 和 tests 为权威来源。
- 当外部 docs 与 Docnav 已有约定冲突时，先保护本仓库 contract，再说明冲突和可选迁移路径。
