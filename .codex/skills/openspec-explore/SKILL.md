...
9. 用户要求沉淀探索结果时，可以创建或更新 OpenSpec artifacts。这属于记录思考，不属于实现应用代码。
10. 未经用户要求，不自动把每个洞察写入 artifact；可以在关键决策出现时询问是否记录。
...
5. 决策记录草案：当用户要求时，更新 proposal、design、tasks 或 spec delta。
