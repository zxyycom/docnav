...
9. 沉淀探索结果和修改已有 change 时，保存支撑当前结论的依据和仍待回答的问题；需要形成可实施设计与任务时再进入 `$openspec-propose`。
10. 只记录用户确认的决策、决策变更和开放问题答案，并按下节的作用域分工写入持久 owner。

## 决策记录

1. 只约束关联 change 的已确认决策写入其 `## Decisions`，使用连续编号并保留旧编号；跨 change 仍有长期影响的判断写入 `$decision-records`。
