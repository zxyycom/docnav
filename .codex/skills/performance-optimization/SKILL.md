...
description: "用于 performance optimization：当 measured budget、baseline、用户报告或 profiling data 指向 CLI/API、parser/navigation、pagination、identifier lookup、subprocess latency、output serialization、CPU 或 memory bottleneck 时使用。"
---
...
2. Isolate：比较 direct implementation、CLI/API wrapper 和 subprocess path，把慢点归类为 parser/domain、routing、IO/process、output、identifier lookup、search、pagination 或 memory。
...
7. Evidence：只有已有 budget、baseline、用户报告或 profiling 结果需要后续比较时，沉淀覆盖优化路径的最小性能验证证据：unit test、smoke check、benchmark note 或 budget 文档。Timing measurement 默认作为 observation；只有明确 budget 或 merge policy 时才成为 required check。

## 参考加载

- 只有性能问题明确落在 Docnav 或类似文档导航、CLI/subprocess/protocol、large-document workload 或 workspace verification 时，读取 [performance-checklist.md](references/performance-checklist.md)。其中包含 scoped baseline 清单、fixture 形状、triage 表、命令模板、budget 模板、常见误判、red flags 和验证范围。
...
- 已运行覆盖改动范围的最小 benchmark/test/smoke；跨语言/runtime、schema/examples、output layer、CLI/API、subprocess 或 docs 边界时优先运行仓库约定的 workspace verification。
- 无法沉淀自动化 timing evidence 时，已记录复现命令、fixture、budget 或 baseline，以及原因。
