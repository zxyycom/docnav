...
只在 Docnav 仓库内处理 public contract、CLI-first navigation、adapter ownership、ref、pagination、schema/example、error mapping 时读取本 reference。通用 REST、GraphQL 或 TypeScript API 设计不使用本文件。
...
- Readable CLI output: `readable-view` 的 header fields、block framing、信息密度、truncation、continuation hint 和人类可读 error text。
...
- Ref: adapter-owned opaque identifier；core 和上层入口只原样传递。
...
- Error mapping: adapter/core error 到 protocol code、CLI exit/readable message 的一致映射。
...
2. Format adapters own format detection、parsing、navigation strategy、ref generation/parsing、pagination result 和 direct adapter CLI behavior。
3. Raw protocol and readable output share business semantics, not transport wrappers.
...
- CLI、adapter、schema/example 或 docs 边界变更：run the relevant smoke/integration checks; for cross-boundary Docnav work, use the repository workspace verifier when feasible.
