...
对 navigation、selection 或 identifier 回归，要重放原始 user-visible path，并按风险检查 readable-view、readable-json 和 protocol-json；非文档 help/version 可单独检查 PlainText。跨语言/runtime、CLI/API behavior、contract、schemas、examples、generated fixtures、docs、bridge mapping 或 smoke coverage 时，优先运行仓库约定的 workspace verification。
