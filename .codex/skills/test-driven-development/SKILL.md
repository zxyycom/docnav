...
说明测试需求时使用当前 contract 语言：`<owner surface> 证明 <observable behavior / invariant / output shape>`。测试名或验证标题按当前行为命名，例如 `MCP bridge readable-error mapping`、`Markdown empty-document outline`、`subprocess fallback classification`。若说不清这个句子，优先选择现有验证、手动复现或不新增自动化测试。
...
Bug fix 的测试名或验证标题使用修正后的当前行为，例如 `readable error details reach MCP structuredContent`。历史失败只作为 setup/evidence，不作为分类或主标题。
