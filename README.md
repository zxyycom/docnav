...
- `docnav` 核心 CLI 与独立 Markdown adapter。
- `docnav adapter list/install/update/remove` 的正式 adapter 管理能力。
- `docnav-mcp` Node.js / JavaScript MCP bridge 的接入层职责。
- 原始 adapter invoke 协议、operation 自描述响应、扁平 outline、ref、page 和结构化错误。
- 普通 CLI 和 MCP 的阅读输出，以及 `adapter invoke` / `docnav --output protocol-json` 的完整协议输出。`readable-json` 是结构化阅读输出，不作为长期机器解析接口。
- Markdown `outline`、`read`、`find` 和 `info` 全部能力；`outline -> ref -> read` 是首要纵向阅读链路。
- JSON、YAML、TOML 和 INI adapter 作为后续格式能力，不属于 v0 首期实现范围。

v0 文档本身不创建实现代码；后续 v0 实现必须完成正式安装、更新、移除和列表管理流程，不使用占位式安装更新算法。首期 adapter 安装来源为 GitHub 链接和本地可执行文件；本地可执行文件必须做 hash 校验。
...
| 审计历史或变更依据 | [OpenSpec change](openspec/changes/archive/2026-06-05-define-docnav-v0-docs/proposal.md) | `openspec/changes/archive/2026-06-05-define-docnav-v0-docs/` 下的 design、specs、tasks |
...
| 类型 | 文档 | 用途 |
| --- | --- | --- |
| 入口导航 | README | 项目目标、v0 范围、术语、角色化阅读路径 |
