...
fn string_regex_and_length_constraints_validate_present_values() {
    ...
    fields
        .extract_without_default(&json!({"defaults": {"slug": "doc-1"}}))
        .expect("valid slug passes");

    let error = fields
        .extract_without_default(&json!({"defaults": {"slug": "Doc"}}))
    ...
    let error = fields
        .extract_without_default(&json!({"defaults": {"slug": "do"}}))
    ...
    let error = fields
        .extract_without_default(&json!({"defaults": {"slug": "docnav-x"}}))
...
fn array_length_constraints_validate_present_values() {
    ...
    fields
        .extract_without_default(&json!({"defaults": {"items": [1, 2]}}))
        .expect("array at the maximum length passes");

    let error = fields
        .extract_without_default(&json!({"defaults": {"items": [1, 2, 3]}}))
