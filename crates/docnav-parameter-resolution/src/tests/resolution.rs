...
fn direct_project_user_default_priority_preserves_source_info() {
    ...
    let resolution = resolve_parameters(&entries, sources, EntryPassthroughPolicy::Retain);
...
fn project_config_overrides_user_config_and_static_default_fills_absent_value() {
    let entries = vec![catalog_entry("docnav.defaults.pagination.limit")];
    let identity = identity("docnav.defaults.pagination.limit");
    let project_resolution = resolve_parameters(
        &entries,
        ParameterSources {
            project_config: source_with_value(&identity, json!(200)),
            user_config: source_with_value(&identity, json!(300)),
    ...
    let default_resolution = resolve_parameters(
        &entries,
        ParameterSources::default(),
        EntryPassthroughPolicy::Retain,
    );
...
#[test]
fn required_missing_value_reports_parameter_resolution_diagnostic() {
...
#[test]
fn passthrough_remains_outside_parameter_resolution_validation() {
    let entries = vec![catalog_entry("docnav.defaults.pagination.limit")];
    ...
    let resolution = resolve_parameters(&entries, sources, EntryPassthroughPolicy::Delegate);
