...
use crate::{
    ...
    construct_direct_input_source_with_passthrough, derive_parameter_catalog,
    load_parameter_config_source, resolve_parameters, ConfigPathOrigin, ConfigSourceLevel,
    EntryPassthroughPolicy, LoadedParameterConfigSource, ParameterCatalogError,
    ParameterConfigSourceDescriptor, ParameterResolution, ParameterResolutionHandoff,
    ParameterSources,
};

#[derive(Clone)]
pub struct ParameterResolutionPipeline<'a> {
...
impl<'a> ParameterResolutionPipeline<'a> {
    ...
    pub fn with_project_config_path(mut self, path: impl Into<PathBuf>) -> Self {
        self.project_config = Some(PipelineConfigSource::Descriptor(
            ParameterConfigSourceDescriptor::new(
    ...
    pub fn with_loaded_project_config(mut self, loaded: LoadedParameterConfigSource) -> Self {
        self.project_config = Some(PipelineConfigSource::Loaded(loaded));
        self
    }

    pub fn with_loaded_user_config(mut self, loaded: LoadedParameterConfigSource) -> Self {
    ...
    pub fn resolve(
    ...
    ) -> Result<ParameterResolution, ParameterResolutionPipelineError> {
        ...
        let mut resolution = resolve_parameters(entries, sources, self.passthrough_policy);
        resolution.extend_diagnostics(diagnostics);
        Ok(resolution)
    }

    fn catalog(&self) -> Result<crate::ParameterCatalog, ParameterResolutionPipelineError> {
        let direct_processing = self.direct_input_processing_id.as_ref().ok_or(
            ParameterResolutionPipelineError::MissingProcessingRole(
...
enum PipelineConfigSource {
    Descriptor(ParameterConfigSourceDescriptor),
    Loaded(LoadedParameterConfigSource),
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum ParameterResolutionSourceRole {
    DirectInput,
    Config,
}

impl ParameterResolutionSourceRole {
...
pub enum ParameterResolutionPipelineError {
    MissingProcessingRole(ParameterResolutionSourceRole),
    Catalog(ParameterCatalogError),
}

impl fmt::Display for ParameterResolutionPipelineError {
...
impl std::error::Error for ParameterResolutionPipelineError {}

impl From<ParameterCatalogError> for ParameterResolutionPipelineError {
    fn from(error: ParameterCatalogError) -> Self {
        Self::Catalog(error)
    }
}
...
) -> (Option<JsonValue>, Vec<ParameterResolutionHandoff>) {
    match config {
        Some(PipelineConfigSource::Descriptor(descriptor)) => {
            load_parameter_config_source(&descriptor).into_parts()
        }
