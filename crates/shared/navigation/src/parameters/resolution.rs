...
const EXPLICIT_FIXED_SOURCE_ID: &str = "explicit-fixed";
...
pub(super) fn resolve(
    ...
    if let Some(input) = direct_input {
        if let Some(source) = extract_direct_source(fields, input)? {
            sources.push(source);
        }
    }
    if let Some(source) = cli_source.filter(|source| !source.candidates().is_empty()) {
...
pub(super) fn source_label(source_id: &SourceId, source_kind: &SourceKind) -> &'static str {
    match source_id.as_str() {
        EXPLICIT_FIXED_SOURCE_ID | crate::DOCUMENT_CLI_SOURCE_ID => "explicit",
...
fn extract_direct_source(
    fields: &FieldDefSet,
    input: &DirectInput,
) -> Result<Option<Source>, NavigationError> {
