...
use crate::{ProcessingId, ProcessingInputKind, ProcessingLocator};
...
pub enum FieldDefSetBuildError {
    ...
    DuplicateProcessingLocator(Box<FieldDuplicateProcessingLocatorError>),
}
...
#[derive(Debug, PartialEq)]
pub struct FieldDuplicateProcessingLocatorError {
    pub processing_id: ProcessingId,
...
impl From<FieldDuplicateProcessingLocatorError> for FieldDefSetBuildError {
    fn from(value: FieldDuplicateProcessingLocatorError) -> Self {
        Self::DuplicateProcessingLocator(Box::new(value))
    }
}
...
impl fmt::Display for FieldDefSetBuildError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::DuplicateProcessingLocator(error) => write!(formatter, "{error}"),
        }
    }
}
...
impl fmt::Display for FieldDuplicateProcessingLocatorError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "processing {} locator {:?} is declared by {} at {} and {} at {}",
            self.processing_id,
            self.locator,
            self.current_identity.as_str(),
            display_declaration_path(&self.current_declaration_path),
...
impl std::error::Error for FieldDuplicateProcessingLocatorError {}
