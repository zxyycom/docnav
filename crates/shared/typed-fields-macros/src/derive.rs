...
pub(crate) fn expand(input: DeriveInput) -> Result<TokenStream2> {
    ...
    Ok(quote! {
        ...
        impl #definition_set_name {
            ...
            pub fn materialize(
                &self,
                values: &::docnav_typed_fields::FieldValueMap,
            ) -> ::std::result::Result<#struct_name, ::docnav_typed_fields::FieldValidationErrors> {
                let values = ::docnav_typed_fields::__private::materialize_values(
                    self.__field_def_set.as_ref(),
                    values,
                )?;
                Ok(<#struct_name as ::docnav_typed_fields::FieldDefs>::__values_from_slots(
                    &values,
                    0,
                ))
            }
