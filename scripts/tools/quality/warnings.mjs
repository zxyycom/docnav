 ...
 * @typedef {import('./schema.mjs').FunctionMetric} FunctionMetric
 * @typedef {import('./schema.mjs').DuplicateCodeFragment} DuplicateCodeFragment
 * @typedef {import('./schema.mjs').WarningRecord} WarningRecord
 * @typedef {import('./config.mjs').DEFAULT_CONFIG} QualityConfig
 */
 ...
 */
export function generateWarnings({ files, functions, duplicates, config, scope, baseline, comparisonStatus }) {
  /** @type {WarningRecord[]} */
  const warnings = [];
  ...
  for (const dup of duplicates) {
    ...
    const uniqueAreas = [...new Set(involvedAreas)];
  ...
  warnings.sort((a, b) => {
    const lvlOrder = { error: 0, warning: 1, info: 2 };
...
function shouldWarn({ isChanged, isWatchlistOnly, value, floor, delta, deltaFloor }) {
...
function basisFor(isChanged, delta) {
...
function buildFileBaselineMap(files) {
  return new Map(files.map((file) => [file.path, file]));
}

function buildFunctionBaselineMap(functions) {
...
function buildDuplicateBaselineIndex(duplicates) {
  const index = new Map();
  for (const dup of duplicates) {
...
function countMatchingBaselineDuplicates(dup, baselineIndex, hasBaselineDuplicates) {
