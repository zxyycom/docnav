 ...
 * ecmascript 在某些安装中对 .mjs + --file-list 组合不工作，
 * 此时节点脚本 code areas 应回退为跳过并记录原因。
 */
const CODE_AREA_LANGUAGE = {
 ...
 */
export function scanWithCpd({ files, cwd, toolConfig, minimumTokens, codeArea, skipIfUnavailable }) {
  if (files.length < 2) {
 ...
 */
export function parseCpdXml(xml, cwd) {
  try {
 ...
 */
export function getCpdVersion({ cwd, toolConfig }) {
...
function spawnPmd(command, args, options) {
...
export function buildPmdShellCommand(command, args) {
...
export function parsePmdVersionOutput(output) {
...
function parseIntegerAttribute(attrs, name) {
 ...
 * @param {string} path
 */
function tryCleanupFileList(path) {
  try {
    unlinkSync(path);
  } catch {
