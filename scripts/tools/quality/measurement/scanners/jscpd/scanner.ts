...
import { runProcess, runProcessSync, type ProcessResult } from "../../../../process.ts";
...
const CODE_AREA_FORMAT: Record<string, string | null> = {
...
export function getJscpdFormatForCodeArea(codeArea: string): string | null {
  return CODE_AREA_FORMAT[codeArea] ?? null;
}
...
function prepareJscpdScan(options: ScanWithJscpdOptions): PreparedJscpdScan {
  ...
  return {
    ...
    cwd,
    toolConfig,
    invocation: prepareJscpdInvocation({ files, toolConfig, minimumTokens, codeArea })
  };
}
...
function prepareJscpdInvocation({
  ...
  const tempDir = mkdtempSync(join(tmpdir(), "docnav-jscpd-"));
  const outputDir = join(tempDir, "report");
  const configPath = join(tempDir, ".jscpd.json");
  const format = getJscpdFormatForCodeArea(codeArea);
  const config = {
    path: files,
    reporters: ["json"],
