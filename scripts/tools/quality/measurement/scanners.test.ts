...
describe("quality scanner output parsing", () => {
  ...
  it("parses jscpd format, version, and JSON output", () => {
    ...
    assert.equal(getJscpdFormatForCodeArea("typescript-production-scripts"), "typescript");
  ...
  it("keeps real duplicate findings non-fatal and normalizes jscpd JSON", () => {
    ...
    try {
      const result = scanWithJscpd({
        ...
        cwd: tempDir,
        toolConfig: DEFAULT_CONFIG.tools.jscpd,
        minimumTokens: 20,
        codeArea: "typescript-production-scripts"
      });
