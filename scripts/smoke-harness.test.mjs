...
import { createSmokeHarness, createSmokeState, resolveSmokeConcurrency } from "./smoke-harness.mjs";
...
describe("smoke harness task scheduling", () => {
  ...
  it("records failed task results without stopping other independent tasks", async () => {
    ...
    assert.equal(state.testResults.find((result) => result.label === "failing task").error.message, "expected failure");
    assert.deepEqual(
      state.commandRecords.map((record) => record.name).sort(),
...
function createHarness(state, events) {
...
function expect(record, condition, summary) {
