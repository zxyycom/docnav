 ...
 */
export function scanWithLizard({ files, cwd, toolConfig }) {
  if (files.length === 0) {
 ...
 *
 * 兼容早期 8 列测试形状：
 * NLOC,CCN,token count,parameter count,start line,end line,file path,function name
 *
 * @param {string} csv
 * @returns {{ ok: true, functions: FunctionMetric[] } | { ok: false, error: string }}
 */
export function parseLizardCSV(csv) {
  try {
    ...
    for (const line of lines) {
      ...
      const shape = parseLizardRowShape(parts);
      const { startLine, endLine, filePath, funcName } = shape;
...
function parseLizardRowShape(parts) {
  if (parts.length >= 11 && isIntegerText(parts[9]) && isIntegerText(parts[10])) {
    return {
      startLine: parseInt(parts[9], 10),
      endLine: parseInt(parts[10], 10),
      filePath: parts[6],
      funcName: parts[7]
    };
  }
  ...
  return {
    startLine: parseInt(parts[4], 10),
    endLine: parseInt(parts[5], 10),
    filePath: parts[6],
    funcName: parts[7]
  };
}
 ...
 */
export function getLizardVersion({ cwd, toolConfig }) {
