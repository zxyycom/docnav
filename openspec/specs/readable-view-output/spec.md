...
Readable-view renderer config 和 conformance vectors MUST 可供非 Rust 实现消费。Conformance 验证 MUST 聚焦语义字段：block pointer、byte length、block payload 还原、header 字段语义、warning 数组一致和空 block 合法。Conformance 断言 MUST NOT 依赖 JSON header object key 顺序、block section 输出顺序或与 Rust 实现逐字节一致。
