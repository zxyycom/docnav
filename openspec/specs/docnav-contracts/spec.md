...
Ref 文档和示例 MUST 把 ref 描述为 adapter 生成和解析的非空字符串；Markdown、全文和其它格式定位语法均属于对应 adapter 私有实现或 adapter 自有兼容契约，不属于共享协议、`docnav` CLI 或 MCP 公共格式。Ref 文档 MUST 说明共享协议、`docnav` 和接入层只原样传递 ref，MUST NOT 解析、拼接、规范化或从 display 推断 ref。

#### Scenario: Ref 规范展示公共边界
- **WHEN** 读者打开 Ref 文档
- **THEN** 能区分 path 负责定位文档、ref 负责定位当前文档内部区域
- **THEN** 能识别格式定位语法由 adapter 拥有，不属于共享协议、`docnav` CLI 或 MCP 公共格式

#### Scenario: Ref 规范保持唯一定位语义
- **WHEN** 调用方从 outline 或 find 取得 adapter 生成的 ref
- **THEN** 调用方把相同 path 和 ref 原样传给 read
- **THEN** adapter 解析 ref 并读取唯一文档区域
- **THEN** ref 无匹配和多匹配分别返回稳定错误

#### Scenario: 文档示例不固定 Markdown ref 拼写
- **WHEN** 实现者更新 Ref 文档、Markdown 示例或相关规范示例
- **THEN** Ref 文档和相关规范文案不把 Markdown adapter 的私有 ref 拼写定义为共享契约
