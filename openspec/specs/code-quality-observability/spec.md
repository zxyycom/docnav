...
Docnav 仓库的代码质量观测能力 MUST 生成当前源码状态的指标快照，并 MUST 将 Lizard、scc 和 PMD CPD 指标结果作为信息性报告处理。指标值本身 MUST NOT 使现有 workspace 验证、CLI 行为、adapter 行为或 OpenSpec 验证失败。Clippy MAY 继续作为 Rust 阻断式 lint gate，因为它保护 Rust correctness 和 lint policy，不属于非阻断观测快照。
...
代码质量观测快照 MUST 至少包含仓库体量、语言占比、文件级总行数、文件级代码行数、文件级复杂度、函数级代码行数、函数参数数量、函数圈复杂度和重复代码片段。报告 MAY 包含额外指标，但额外指标 MUST 不改变首期核心指标的字段语义。

#### Scenario: 文件级指标
- **WHEN** 观测命令扫描一个纳入范围的源码文件
- **THEN** 快照记录该文件的规范化路径
- **THEN** 快照记录该文件的总行数和 scc `Code` 代码行数
...
- **THEN** 快照记录该单元所属文件和名称
- **THEN** 快照记录该单元的 Lizard `NLOC` 代码行数
- **THEN** 快照记录该单元的参数数量
...
#### Scenario: 生成 JSON 明细
- **WHEN** 观测命令完成扫描
- **THEN** 机器可读输出包含 schema version、扫描元数据、工具名称和版本、扫描范围、排除规则、code areas、current 扫描输入指纹、可选 baseline metadata、baseline status、comparison status、文件指标、函数指标、重复代码指标、聚合指标、可选 baseline delta records 和 warning channels
- **THEN** 输出路径使用仓库相对路径或规范化路径
- **THEN** 输出不包含依赖目录、构建产物或缓存目录的指标记录
- **THEN** 输出显式记录每个 code area 的 current snapshot、可选 baseline snapshot、delta 可用性、`baseline.status` 和 `comparison.status`
...
- **THEN** 默认未启用 baseline comparison 时，`warnings.changed[]` 和 `warnings.regressions[]` 为空数组
- **THEN** 显式启用 baseline comparison 时，`warnings.changed[]` 只包含可用于 CI annotation 的 changed-scope warning records
- **THEN** 显式启用 baseline comparison 时，`warnings.regressions[]` 只包含 changed warnings 中有 baseline delta basis 的退化记录
...
- **THEN** summary MUST 展示 Warnings section；当本次 changed scope 数据可用时，summary MAY 展示 risk-ranked Changed Files Watchlist
- **THEN** Changed Files Watchlist MUST 只展示命中风险条件的 changed files
- **THEN** 风险条件 MUST 来自当前可用证据，至少包含命中当前全量 warning、可用 baseline delta 或命中重复代码
- **THEN** Changed Files Watchlist MUST NOT 依赖 `warnings.changed[]` 作为唯一风险来源，因为该 channel 是 CI annotation 输入，默认未启用 baseline comparison 时可以为空
- **THEN** 未启用 baseline comparison 时，Changed Files Watchlist MUST NOT 将缺失的 baseline delta 描述为退化
- **THEN** Changed Files Watchlist MUST 限制展示数量，并说明 changed files 总数和展示数量，例如 `Changed files: 63 total, 10 shown by risk ranking`
- **THEN** summary MUST 展示全量 warnings 摘要，并在 changed warnings 可用时单独展示 changed warnings
...
- **THEN** 每条 warning record 包含 level、rule id、source tool、normalized path、code area、changed scope、optional line、metric name、metric value、comparison basis、nullable baseline value、nullable delta value 和 message
...
- **THEN** `warnings.ndjson` 的记录来源 MUST 是 `warnings.changed[]`；默认未启用 baseline comparison 时可以为空，用于 CI annotation 的降级输入
...
- **THEN** scc 文件大小 warning MUST 使用 `Code` 代码行数作为阈值输入，而不是包含注释和空行的总行数
- **THEN** Lizard 函数大小 warning MUST 使用 `NLOC` 代码行数作为阈值输入
- **THEN** PMD CPD `minimum tokens` MUST 按 code area 配置，而不是使用单一全仓库默认值
- **THEN** PMD CPD cache miss task 的最大并发数 MUST 由配置控制
...
代码质量观测能力 MUST 将当前快照、动态 warning、显式 opt-in 的 baseline delta、未来自动报告和未来门禁策略分离。阻断阈值、PR 差异阻断或豁免机制 MUST 由后续 change 明确定义，不得从首期观测结果隐式推导。
...
- **THEN** warning MAY 使用 changed scope、显式 opt-in 的 baseline delta 和 absolute floor 来避免报告噪音型历史债务
