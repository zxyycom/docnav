...
- **AND** the definition exposes declared extraction strategies keyed by caller-owned strategy id
- **AND** a JSON path extraction strategy exposes structured JSON path segments that can derive display, schema, or validation paths
- **AND** the definition exposes the extraction input kind required by each strategy id
...
- **THEN** it can obtain field-level facts such as type, requiredness, null-as-absent policy, default metadata, enum values, numeric range constraints, regex constraints, length constraints, and extraction strategy metadata
...
- **AND** the set can expose an `extract(strategy_id, input)` function, an `extract_with_static_defaults(strategy_id, input)` function, a `validate(strategy_id, input)` function, a `validate_with_static_defaults(strategy_id, input)` function, a value kind view, a typed default values object, and a schema metadata view
...
- **AND** extract and validate failures use the same field extraction error type, wrapping field validation failures when extraction reaches field validation
...
- **AND** `extract(strategy_id, input)` does not apply static defaults
- **AND** `extract_with_static_defaults` applies static defaults only when the input omits a field
- **AND** `extract_with_static_defaults` returns the same typed values object shape as `extract`
...
- **AND** each leaf `FieldDef` builder supplies canonical field identity, one or more extraction strategies, validation, and default metadata
...
- **AND** missing leaf extraction strategies fail during set build
- **AND** the same strategy id cannot require different input kinds within one definition set
