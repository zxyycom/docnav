...
## Purpose
Define the typed field definition core: reusable field identity, structured JSON path metadata, value kind and constraint metadata, static default metadata, field-level decode/validation attribution, schema metadata views, and duplicate identity checks. Consumer owners remain responsible for CLI/config source resolution, operation binding, manifest/probe policy, protocol envelopes, readable output, and complete JSON Schema documents.
## Requirements
