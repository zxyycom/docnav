...
- successful MCP structuredContent 映射 readable success payload 字段；失败时映射 readable error 中的 primary `DiagnosticRecord`，不复制完整 protocol envelope。
...
- 子进程成功退出时，stderr 中的 owner-scoped status 不升级为 MCP 错误；bridge 以 readable JSON payload 和退出码决定成功/失败。
