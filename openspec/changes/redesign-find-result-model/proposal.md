...
- Record a contract handoff to the Current `json-adapter` owner and a separate raw-facts/presentation handoff for `add-json-readable-renderer`. The archived `add-json-adapter` artifacts remain immutable; whether the approved shared find model requires a complete `json-adapter` delta is decided only when the human model gate closes.
...
- No pre-gate modification of JSON-specific find or readable presentation. The archived adapter change remains untouched; after the model gate closes, this change must assess the Current `json-adapter` owner and add a complete delta only if the approved shared contract requires one. Renderer presentation remains with `add-json-readable-renderer`.
...
- `add-json-adapter` is archived, and the Current `json-adapter` capability now owns JSON navigation behavior. Its source-oriented search and canonical ref decisions constrain the eventual model handoff. This change does not alter that owner before approval, does not modify the archive, and leaves JSON presentation to `add-json-readable-renderer`.
- `redesign-token-cost-estimation` remains semantically independent even though both changes fully replace some of the same owner requirements. Whichever change lands later must refresh its own overlapping `MODIFIED` bodies from the then-Current owners and preserve every already-effective clause from the earlier change.
