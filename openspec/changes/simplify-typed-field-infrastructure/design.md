...
| Owner | Owns | Consumes |
| --- | --- | --- |
| Core parameter catalog | 接受哪些产品参数、source locators、standard value kind、default、merge strategy、operation binding、可选的精确 adapter-id 标记和 core validation rules | static adapter ids 与 shared typed-field mechanics |
...
5. Navigation 从同一 resolution result 构造 protocol projection 和 closed standard operation input。
...
2. Catalog entry 提供 canonical identity、适用的 CLI/env/config locators、standard value kind、default、merge strategy、operation binding、可选的精确 adapter-id 标记，以及 core 选择执行的 validation facts。
3. Catalog construction 拒绝 duplicate identity/locator、指向 unknown adapter id 的标记、missing binding、invalid operation binding 与 incompatible definition。
...
6. Protocol `OperationArguments.options` 和 standard operation input 是同一 resolution result 的并列 projection；strategy input 不从 protocol JSON 反向构造。
7. Standard operation input 只暴露 compile-time typed fields/accessors 或 closed variants，并包含策略所需的 normalized operation facts；strategy 不接收通用 parameter bag、protocol envelope、source-priority metadata 或 parameter declarations。
...
### 4. Protocol and strategy input are sibling projections

Navigation 从 typed resolution result 分别构造稳定的 protocol shape 与 operation-specific closed typed input。Standard input 类型位于现有 shared operation contract，以便 navigation 构造并由 adapter strategy 消费；这种物理依赖位置不转移 core 对字段、binding 和 accepted input surface 的 ownership。每个策略可见值通过静态字段、typed accessor 或 closed variant 暴露，protocol representation 和通用 parameter bag 都不作为 strategy input。
...
等价测试通过后，runtime 只通过 core catalog 构造产品参数和 standard input。实现与验证不需要维护第二套 parameter definition source。
...
2. 建立并验证 core catalog、可选的精确 adapter-id 标记、closed standard operation input、env activation rule 和 layered validation boundary。
...
5. 从同一 resolution result 直接构造 protocol projection 与 standard operation input。
...
1. Core catalog 是 CLI、env/config extraction、config inspect、navigation resolution 与 standard-input binding 的唯一产品参数 definition source；full validation 与 selected-operation view 从同一 facts 派生，且 untagged、matching tag 和 non-matching tag 的结果符合静态规则。
