...
- [Risk] 元数据日志仍可能放大本地路径、query 或 ref 形状。Mitigation: path 默认只记录 bounded display/hash。
  Query/ref 默认记录 bounded value、presence/length/hash/summary，而非无界原始值。
...
- 摘要策略：document path 的 bounded display/SHA-256 hash 规则。
- 其它摘要策略：query、ref、diagnostics 和 content 的 bounded value、presence/length/SHA-256 hash/truncated summary 规则。
