...
- [x] 1.2 确认 typed-field definition 是字段事实 owner，standard parameter resolution 只消费字段 identity、mapped value processing strategy、schema metadata、默认值、typed value 校验能力和 caller processing result。
...
## 2A. Typed-Field Processing Strategy 基础

- [x] 2A.1 删除 `FieldDefBuilder::path` 兼容入口，叶子字段通过 `FieldDefBuilder::process(processing_id, strategy)` 声明处理来源。
- [x] 2A.2 新增 `FieldDefBuilder::process(processing_id, strategy)`，允许同一字段按 processing id 声明不同处理策略。
- [x] 2A.3 在 `FieldDefSet` build 阶段校验同一 processing id 在同一 definition set 内只对应一种 input kind。
- [x] 2A.4 新增 JSON path processing strategy 的 `fields.process(processing, json)`、`process_with_static_defaults(processing, json)` 和对应 validate typed object projection。
- [x] 2A.5 建模 Rust field processing input kind，后续 direct-input derive projection 不在本 slice 展开。
- [x] 2A.6 新增 typed-field processing build：caller 通过 processing id 和处理函数把该 id 的 raw input 转为处理值，typed-field 在同一 processing id 下返回 extraction result 与 caller processing result。

## 2B. Source Construction 与 Config Loading

- [x] 2B.1 定义 processing-specific metadata projection，包含 standard parameter identity、processing id、path segments、value kind、constraints 和 default metadata。
...
- [x] 2B.3 实现 direct input source construction：调用方提供已结构化的 CLI/invoke input，标准参数层按 catalog/index 映射 `source=direct`，passthrough processing result 进入 passthrough handoff。
...
- [x] 2C.2 在 pipeline 中建模 direct/config 两个 source role 到 typed-field processing id 的绑定；default 来自 typed-field default 和 dynamic defaults，不作为 processing role。
- [x] 2C.3 从 `FieldDefSet::schema_metadata()`、`FieldDefSet::processing_metadata("direct")` 和 `FieldDefSet::processing_metadata("config")` 内部形成 catalog/index，复用 source path conflict 检查。
...
- [x] 3.6 添加 source construction fixture，证明 direct input、project config、user config 和 default mapped values 都由 catalog/index 与 processing strategy 构造，passthrough 可由 caller processing result 交接。
...
- [x] 3.8 添加 passthrough fixture，证明 direct/project/user passthrough processing results 按 source scope 返回且不参与 typed validation。
...
- [x] 3A.1 添加 typed-field fixture，证明同一 processing id 的 JSON path processing strategy 可抽取 typed object。
- [x] 3A.2 添加 typed-field fixture，证明同一 processing id 混用 JSON input kind 和 Rust field input kind 时 build 失败。
- [x] 3A.3 添加 typed-field fixture，证明 processing build 接受 typed raw input 并原样返回 caller 处理值。
- [x] 3A.4 添加 typed-field fixture，证明 `process` 同时返回 extraction result 和 caller processing result，且 extraction 失败时仍保留 processing result。
- [x] 3.10 添加 pipeline fixture，证明普通调用者只提供 `FieldDefSet`、direct/config processing id 和输入 sources，不需要手工构造中间 catalog/index。
