...
- 不定义通过 CLI string 传递或解码任意 JSON 的通用语法；`ValueKind::Json` 不是 clap companion 支持的 CLI projection。
...
`cli-config-resolution-clap` 根据 CLI extraction strategy 注册或读取 clap arguments；支持的 projection 固定为 string、integer、finite number、`SetTrue` boolean、repeated string array 和 repeated `key=value` object。`ValueKind::Json` 不具有隐式 CLI string syntax；当 canonical field 把 JSON value kind 与 CLI locator 组合时，companion 必须返回 `ClapProjectionError::UnsupportedValueKind`，不得把 raw flag string 解码为任意 JSON。该 capability check 保持在 companion boundary，typed-fields 和 framework-independent core 不复制 clap-specific policy。未注册 flag 的拒绝沿用 clap 原生行为。

Env extractor 接收 `IntoIterator<Item = (String, String)>` 或等价可测试输入，只查询声明过的 env name。Config companion 首批以当前 `serde_json::Value` structured document 为事实边界，只查询声明过的 config path。
