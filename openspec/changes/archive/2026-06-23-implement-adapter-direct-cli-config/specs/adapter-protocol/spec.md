本 delta 是该 change 的正式 OpenSpec delta，定义 adapter direct CLI 配置路径参数、配置读取、合并和 native options 显式化的共享 SDK 契约；实现阶段按 tasks 同步 owner 主规范、代码、schema/example 和测试。
