一句话核心：实现正式 adapter 管理，让外部 adapter 的来源、manifest、协议兼容和本地文件 fingerprint 都可审计。
...
- [ ] 1.4 （阻塞：等待 0.1 用户审计确认）实现 `docnav adapter list`，展示身份、版本、格式、协议范围、来源、项目策略命中、install mode 和上次可用状态。
...
- [ ] 2.6 （阻塞：等待 0.1 用户审计确认）安装时执行 manifest，校验 manifest schema 和协议兼容性。
...
- [ ] 3.2 （阻塞：等待 0.1 用户审计确认）确保 update 在 manifest、schema、协议和 fingerprint 校验通过后才替换旧记录。
