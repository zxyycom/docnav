...
`docnav adapter list` MUST 输出已安装 adapter 的 manifest 身份、version、支持格式、协议范围、install mode、安装来源、项目策略命中和上次可用状态。默认 list MUST NOT 重新计算 fingerprint；显式健康检查模式 MAY 刷新 fingerprint 和可用状态。
...
`docnav adapter update [adapter-id] [--version <version>]` MUST 使用已记录来源获取或重新验证候选版本，并 MUST 在 manifest、schema、协议兼容和 fingerprint 校验全部通过后才替换旧记录。
