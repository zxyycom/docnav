...
- 安装和更新时执行 manifest，校验当前 schema、必需字段、字段类型和语义。
...
3. adapter resolver 先合并 registry，再执行健康与当前契约校验。
   - resolver 从用户级安装 registry 读取 id/version/command。
   - resolver 应用项目级 allowlist、denylist 和当前版本选择。
   - resolver 丢弃 registry 中已标记不可用、显式健康检查已发现 fingerprint 失配或 manifest 当前契约校验失败的版本。
...
6. 内置下载来源必须通过简写解析到具体可执行制品。
   ...
   - 下载得到的可执行制品在 manifest 当前 schema 和语义校验通过后进入 managed artifact 目录。
...
7. update 使用旧记录来源并采用先验证后替换。
   - 新候选 manifest 当前 schema、语义和 fingerprint 全部通过后才替换旧记录。
   - 失败时旧版本保持可用，并返回结构化错误。
...
- 当前版本自动选择的确定性规则需要实现时固定，例如优先选择同一 adapter id 下最新健康版本；一旦固定，必须写入 CLI 主规范和测试。
