...
`docnav-adapter-sdk` MUST 为 adapter direct CLI 提供 AI 友好的宽松 argv 解析。SDK 必须使用 `clap` 或 `clap` builder API 作为共享命令、子命令、固定参数、默认值、枚举值和 help 的解析基础。

Adapter SDK 入口必须保持以下分层：

- Direct CLI 文档操作通过 `clap` 加受控宽松收集解析 argv。
- Adapter `invoke` 通过严格 protocol/schema 校验解析 stdin JSON。
- 传输层解析成功后，direct CLI 文档操作和有效 invoke request 必须映射到同一套 canonical document operation input 或等价 request 结构。
...
- Malformed invoke JSON、未知字段、缺失字段或类型错误必须在进入 canonical document operation input 前失败。
- Readable warning item 必须使用稳定 warning envelope。CLI argv warning 必须使用 `kind: "cli_argv_ignored"`。
...
- **THEN** SDK 继续构造 canonical document operation input 和对应 operation request
- **THEN** 命令结果由业务处理和输出模式决定
- **THEN** SDK 可以输出阅读层 warning 或 stderr 诊断
...
- **THEN** 两者在传输解析后进入同一套 canonical document operation input 或等价 request 结构
...
- **THEN** SDK 不因该参数单独失败
- **THEN** SDK 可以将该参数记录为阅读层 warning 或 stderr 诊断
- **THEN** SDK 不要求该参数值通过当前 operation 不会使用的业务校验
...
- **THEN** stdout 可以在正常阅读文本后拼接用户可理解 warning 文本
- **WHEN** adapter direct CLI 以 readable-json 输出模式成功并存在 warning
- **THEN** stdout payload 可以包含顶层 `warnings` 数组
- **THEN** warning item 包含稳定 `kind`、非空 `reason`、`ignored_tokens` 数组和可选 family-specific 字段
- **THEN** CLI argv warning 使用 `kind: "cli_argv_ignored"`
