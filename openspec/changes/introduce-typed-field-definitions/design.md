...
| Field validation | 聚合 value kind 和字段级 constraints，例如 string/integer/number/boolean/array/object、requiredness、enum、numeric range、string regex、string/array length 或 allowed null；`FieldValidation::int()` 等构造函数直接声明 runtime value kind 和 Rust value type，integer range 使用 integer bound，number range 使用 floating bound；`FieldValidation::string_enum::<T>()` 可由真实 Rust enum metadata 生成 string enum constraints，空 enum allowed set 在 build 阶段失败。 |
...
9. Field definition derive macro 提供 Rust struct 作为常见声明路径，并以 `FieldDef::builder(...)` 作为叶子字段定义。
   ...
   - Consequence: derive macro 不从 validation expression 反推 Rust type 或 requiredness；声明的 leaf type 与 `FieldDefBuilder<T>` 由 Rust 编译器检查一致，因此 validation helper function 可以作为叶子 builder 的一部分使用。Rust field 中的 `T`/`Option<T>` 是 generated value shape 的唯一来源，并在 definition set build 阶段与实际 field metadata 校验一致；static default metadata 不参与 values object type 推导。
