...
- [x] 0.1 用户审计确认：用户已审计本 change 的 proposal、design、spec 和 tasks，并明确允许开始实现；未完成本项前，1.x 及后续任务全部处于阻塞状态。
...
- [ ] 2.3 （阻塞：等待 0.1 用户审计确认）更新验证入口，使发布包验收链路先打包，再从统一 package 目录读取产物；workspace verify 可继续包含明确标识的开发期 smoke。
