...
- [x] 1.1 新增 `crates/docnav` workspace member，提供 package/bin 名称均为 `docnav` 的核心 CLI 可执行入口。
  验收：workspace 可构建 `docnav` package，`docnav` 可解析顶层命令。
- [x] 1.2 建立核心 CLI 命令结构，覆盖 `outline/read/find/info/init/doctor/version/config`。
  验收：每个命令都有确定的参数入口和稳定错误路径。
- [x] 1.3 实现 core 通用参数解析：path、ref、query、page、limit_chars、output 和 adapter。
...
- [x] 1.5 实现项目配置、用户配置和内置默认值读取，支持 `defaults.adapter`、`defaults.limit_chars` 和 `defaults.output`。
  验收：显式参数优先于项目配置，项目配置优先于用户配置，用户配置优先于内置默认值；`defaults.adapter` 只在未传 `--adapter` 时参与预选。
- [x] 1.6 实现 `config get|set|unset|list` 的 MVP 行为。
  验收：`set/unset` 默认写项目配置，传 `--user` 写用户配置；`list` 输出所有支持 key 的当前生效值和来源；未知 key 返回 `INVALID_REQUEST`。
- [x] 1.7 实现 `config list --path <path> [--operation outline|read|find|info]`。
...
- [x] 2.5 实现 manifest/probe 当前契约校验。
...
- [x] 3.1 启动 adapter manifest、probe 和 invoke 子进程。
...
- [x] 3.6 实现错误 code、details、guidance 和退出码映射。
  验收：输入错误、文档/ref/格式错误、protocol/adapter 进程错误和内部错误映射到稳定 code 与主规范退出码。
- [x] 3.7 实现选中 adapter invoke 校验失败路径。
...
- [x] 4.1 新增核心 CLI smoke 脚本。
  验收：使用真实 `docnav` 二进制和真实 adapter 子进程覆盖 `docnav outline -> ref -> read`。
- [x] 4.2 将核心 CLI smoke 接入 workspace 验证入口。
