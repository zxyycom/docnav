...
- [ ] 1.4 （阻塞：等待 0.1 用户审计确认）确保 page 省略时显式转换为 `1`，limit_chars 为有限正整数。

## 2. Path 与 Adapter 选择

- [ ] 2.1 （阻塞：等待 0.1 用户审计确认）实现项目根发现、path 规范化和项目边界检查。
- [ ] 2.2 （阻塞：等待 0.1 用户审计确认）实现 adapter registry 读取接口，先支持测试/临时记录，后续可接正式管理记录。
- [ ] 2.3 （阻塞：等待 0.1 用户审计确认）实现显式 format/content type 候选校验阶段。
- [ ] 2.4 （阻塞：等待 0.1 用户审计确认）实现扩展名候选校验阶段。
- [ ] 2.5 （阻塞：等待 0.1 用户审计确认）实现全量 probe 阶段、FORMAT_UNKNOWN 和 FORMAT_AMBIGUOUS。
...
- [ ] 4.2 （阻塞：等待 0.1 用户审计确认）覆盖 format 选择、content type 选择、扩展名选择、全量 probe 和歧义测试。
