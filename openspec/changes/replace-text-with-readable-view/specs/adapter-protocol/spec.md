...
- Direct CLI document operation 的阅读输出必须通过共享 readable payload 和 readable-view renderer 生成；SDK 不得暴露 document text output mode、text formatter trait 或 text-specific warning writer。
- Manifest、probe、help 和其它非 document operation 通道不得被误归入 document output mode；它们保持各自既有结构化或纯文本边界。
...
- **THEN** stdout 不在 block 后拼接独立 warning 文本
...
#### Scenario: document text direct output 被删除
- **WHEN** 调用方对 adapter document operation 传入 `--output text`
- **THEN** SDK 返回输入错误
- **THEN** help 只列出 readable-view、readable-json 和 protocol-json
- **THEN** adapter 不提供私有 document text formatter 处理该请求
...
- **THEN** 该输出不得重新暴露 document `--output text`
