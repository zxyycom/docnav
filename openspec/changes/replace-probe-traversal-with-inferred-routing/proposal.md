...
- Cross-change gates：`add-project-wide-find` 必须在实施前接受本 change 的 filter/local/fatal classification，并把本 change 记录为 predecessor；`reuse-adapter-document-state` 必须接受或拒绝 candidate traversal/cleanup 缩减 handoff，同时保留 selected document view、full-read、nested read、cleanup 和 private-state价值；`add-ast-grep-code-adapter` 必须接受 exact multi-format mapping 与 no-probe handoff。三项 owner acceptance 都阻断本 change 实施。`audit-runtime-performance-boundaries` 只接收 measurement handoff；archived JSON owner 只提供 probe/TOCTOU migration input。
