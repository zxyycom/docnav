本 change 的目标是将 OpenSpec capability 命名迁移到长期稳定的主 spec ID；本文是未审核临时 delta spec，只存在于 `openspec/changes/normalize-openspec-capability-names/`，不改变现有主规范、主 specs、schema、examples、实现代码或其它 change。
...
### Requirement: Capability ID validation prevents regression
仓库 MUST 提供自动化检查，验证主 specs 和 active delta specs 的 capability ID 符合命名规则，并验证 proposal Capabilities 与实际 delta spec 目录一致。

#### Scenario: Invalid capability ID is introduced
- **WHEN** 主 specs 或 active delta specs 中出现包含 `implementation`、`implement`、`change`、`task`、日期或临时版本阶段的 capability ID
- **THEN** 校验 MUST 失败并指出具体路径

#### Scenario: Proposal and delta specs diverge
- **WHEN** proposal Capabilities 声明的 capability ID 与 `specs/<capability>/spec.md` 目录不一致
- **THEN** 校验 MUST 失败并指出缺失或多余的 capability ID
