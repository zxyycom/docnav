...
实现栈采用 Rust-first：`docnav` core CLI、格式 adapter、`docnav-protocol` 和 `docnav-adapter-sdk` 的主实现使用 Rust；JavaScript/Node.js 仅用于 `docnav-mcp` 薄接入层、AJV schema 校验和文档/示例验证脚本；Python 仅作为开发辅助或 fixture 生成工具，不进入核心运行链路。

## Goals / Non-Goals

**Goals:**

- 以 Rust crate 实现 `docnav-protocol`，覆盖 protocol request/response、manifest、probe、operation result、page 和稳定错误。
- 以 Rust crate 实现 `docnav-adapter-sdk`，让 adapter 可以用统一方式处理 manifest、probe、invoke 和直接命令分发。
