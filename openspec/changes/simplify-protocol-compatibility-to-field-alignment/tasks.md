...
- [ ] 1.1 更新 `docs/protocol.md`，保留固定 version 识别字段，移除协议版本区间协商和最高兼容版本选择语义。
...
- [ ] 2.1 更新 `docs/schemas/manifest.schema.json`，移除 manifest `protocol.min/max` 和 `recommended_parameters` 字段。
...
- [ ] 3.3 更新 request/invoke 校验逻辑：`protocol_version` 只按当前 schema 字段校验，不做范围协商。
- [ ] 3.4 更新 adapter SDK manifest、probe 和 invoke 边界测试，覆盖字段 shape 校验失败而不是版本范围失败。
