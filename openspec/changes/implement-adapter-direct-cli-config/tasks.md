...
- [ ] 2.2 更新 `docs/cli.md`，记录 adapter direct CLI JSON 配置默认路径、配置路径覆盖参数、优先级、支持 key、错误边界和 `invoke` 不读配置。
...
- [ ] 3.1 在 `docnav-adapter-sdk` direct CLI 层新增配置路径参数模型，包含项目级配置路径和用户级配置路径的默认值与覆盖值。
...
- [ ] 3.5 实现配置合并优先级：显式 argv > 项目级配置 > 用户级配置 > 内置默认值，并把结果写入标准 direct CLI operation 参数。
- [ ] 3.6 使用 `NativeOptionSpec` 校验 `options` key、value 和 operation 适用性，并只把当前 operation 适用的 native options 写入最终 operation input。
- [ ] 3.7 实现配置错误诊断，覆盖 JSON parse failure、unknown key、invalid limit_chars、invalid output、invalid native option value 和配置路径不可读。
...
- [ ] 4.2 支持 `.docnav/docnav-markdown.json` 和用户级 `docnav-markdown.json` 的 `defaults.limit_chars`、`defaults.output` 和 `options.max_heading_level`。
...
- [ ] 5.2 增加 `docnav-markdown` CLI smoke fixture，覆盖项目级配置、用户级配置、配置路径覆盖、显式 argv 覆盖和内置默认 fallback。
- [ ] 5.3 增加负向矩阵测试，覆盖非法 JSON、未知 key、非法 `defaults.limit_chars`、非法 `defaults.output` 和非法 `options.max_heading_level`。
