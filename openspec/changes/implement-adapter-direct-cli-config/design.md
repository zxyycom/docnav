...
2. **配置路径是标准 direct CLI 参数，默认值可覆盖。**
   ...
   SDK 为每个 adapter direct CLI 计算两个默认配置路径：项目级默认是当前项目根下 `.docnav/<adapter-id>.json`，用户级默认是用户配置目录下 `<adapter-id>.json`。Direct CLI 可以通过 SDK 标准参数覆盖这两个路径；路径参数只影响配置加载，不进入 protocol request、native options 或 operation handler。
...
5. **配置内容先合并为标准 direct CLI 参数。**
   ...
   SDK 解析 argv 后先得到配置路径、显式业务参数和显式 native options；随后读取用户级与项目级配置，把配置字段合并进标准 direct CLI 参数模型：`limit_chars`、`output`、page、ref/query/path 和 adapter native options。后续 operation request 构造只消费这个标准模型。
...
7. **配置错误是输入错误，不走协议 envelope。**

   Direct CLI 在 document operation handler 执行前发现配置文件 JSON 无法解析、未知 key、非法 value 或不支持的 output 时，返回输入/config 错误。`protocol-json` 输出模式如果能从 argv/config 确定，可以使用当前 direct CLI 错误路径；否则 stderr 诊断和非零退出足够，具体错误 shape 由 adapter SDK 文档定义。
...
- [Risk] 暴露配置路径覆盖参数后，调用方可能把路径参数误认为业务参数。  
  Mitigation: docs 和 help 明确配置路径参数只影响配置加载；它们不进入 protocol request、readable output 或 adapter native options。
...
1. 更新 docs 和 OpenSpec 主规范，明确 adapter direct CLI JSON 配置路径默认值、覆盖参数、配置范围和优先级。
