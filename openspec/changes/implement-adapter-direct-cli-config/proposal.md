本 change 是 adapter direct CLI 配置读取的正式实现计划：通过 SDK 支持可覆盖的项目级/用户级配置文件路径，并把配置内容合并为标准 direct CLI 参数来源对象；实现阶段按 tasks 同步 owner 主规范、代码、schema/example 和测试。
...
- SDK 为两个配置文件路径提供默认值：项目级默认指向项目根下 `.docnav/<adapter-id>.json`，用户级默认指向 SDK 配置参数给出的默认用户配置目录下 `<adapter-id>.json`；该默认用户配置目录未显式提供时使用当前调用位置（启动 cwd），调用方仍可以用路径参数覆盖两个配置文件路径。
- Adapter direct CLI document operation 使用一条内部参数归一化和 operation 执行线路；argv、配置文件和 `invoke` stdin JSON 都只是参数来源，进入业务逻辑前必须显式化为 direct CLI operation 参数。
- Adapter direct CLI 文档操作先解析配置路径参数并读取两个配置文件，再按“显式 argv > 项目级 adapter 配置 > 用户级 adapter 配置 > 内置默认值”合并 `limit_chars`、`output` 和 native options 参数来源。
- Adapter direct CLI 在进入 document operation request construction 或 operation handler 前，将 argv/config/builtin 合并为标准 direct CLI 参数来源对象；配置只投影 `defaults.limit_chars`、`defaults.output` 和完整 `options` object，`path`、`ref`、`query` 与 `page` 仍来自 argv 或入口固定默认。
...
- Adapter direct CLI 把配置文件读取结果按优先级合并为标准 direct CLI 参数来源对象；配置读取层只处理配置源、固定字段投影和来源优先级，后续标准参数处理链路负责类型、范围、枚举、native option 注册和 operation 适用性处理。
...
- 受影响代码：`crates/docnav-adapter-sdk` direct CLI config path parameters、project config discovery、default user config directory parameter、JSON parsing、standard operation parameter source merge、native option default resolution，以及 `crates/docnav-markdown` direct CLI wiring。
