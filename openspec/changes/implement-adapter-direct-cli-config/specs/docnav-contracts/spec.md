本 delta 补强 Docnav 共享契约中的 CLI 配置域隔离和 adapter direct CLI 配置边界；除本 change 明确新增的配置 schema/example 参考材料外，它只在 `openspec/changes/implement-adapter-direct-cli-config/` 下形成未审核临时文档，不改变现有主规范或实现语义。
...
- **THEN** adapter direct CLI 仍按配置源读取、字段映射和标准参数处理链路处理本次调用
