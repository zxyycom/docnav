...
- [x] 1.2 盘点 full migration surface：记录 `protocol-json`、manifest、probe、readable output、stderr、exit behavior、schema、examples、fixtures、consumer tests、generator scripts 和 generated files 的切换范围；projection/details 映射作为内部实现处理，只在 observable field、通道或验证材料变化时同步对应 owner。
...
- [x] 2.3 更新 `docs/standard-parameters.md` 中 validation/source-skipped/ignored argv 等诊断 handoff 边界，使标准参数层输出错误通道记录或可直接入栈的数据。
- [x] 2.4 更新 `docs/testing.md`、`docs/testing/case-maintenance.md` 和覆盖矩阵，明确错误通道语义、canonical details、projection、schema/example/fixture 和 consumer tests 的证明目标。
- [x] 2.5 更新受影响 JSON Schema、examples 和 fixtures，使它们校验 surface projection，不成为 code/details 规则来源。
- [x] 2.6 删除 `docs/protocol/error-rules.json` 的文档入口和生成源说明；若保留 machine-readable projection artifact，标注为 generated/derived validation material。
...
- [x] 3.4 为 `DiagnosticCode` 暴露 category、default severity/effect、details rule 和 surface projection rule；projection rule 不拥有文案或 stdout/stderr placement，具体 field 映射由 boundary 内部投影到对应 owner contract。
...
- [x] 4.4 迁移 adapter direct document operation，使 readable/document protocol 输出与 core 共享同一错误通道 handoff。
...
- [x] 4.9 更新 `protocol-response.schema.json`、readable schema 和示例，使 schema 只校验 surface projection。

## 5. 验证

- [x] 5.1 添加 unit tests，证明 recoverable diagnostics 跨 parser/runtime/output 边界后仍可按 id 或 snapshot 取出，且不会由错误通道自身阻断有效 operation。
...
- [x] 5.4 添加 `DiagnosticCode` tests，证明每个 family variant 都能通过顶层 code 取得 category、默认 severity/effect、canonical details rule 和 projection rule。
- [x] 5.5 添加 details tests，证明每个 code 的 canonical details 缺字段、错类型和多余字段行为符合契约。
- [x] 5.6 添加 projection tests，证明 fatal `DiagnosticCode` 可投影为 protocol/readable error code/details/guidance 和 exit behavior，且栈记录不依赖旧 stable error object。
- [x] 5.7 添加 output tests，证明 `protocol-json`、manifest、probe、readable output 和 stderr 使用错误通道记录作为事实源。
- [x] 5.8 添加 adapter SDK tests，证明 manifest/probe/invoke 诊断从错误通道 flush 或 projection 产生。
- [x] 5.9 添加 readable tests，证明 `readable-view` 和 `readable-json` 从错误通道记录投影 warning/error，并且不会引入目标文档未声明的 `diagnostic_only` effect。
- [x] 5.10 添加 generator/check tests，证明 `docs/protocol/error-rules.json` 已删除，schema/validator/generated constants 不再从它读取，并且 protocol/readable projection 与 `DiagnosticCode` rules 一致。
