...
- Core 维护一个统一静态 adapter registry，作为 adapter id、metadata、capabilities、需求声明和 adapter layer implementation 的 compile/package-time 事实源。
...
`docnav-navigation` 是 SDK 退场后的内部调配层命名，用于集中调配 `outline/read/find/info` 等流程；`docnav-adapter-contracts` 承载 adapter layer interface definitions 和共享 contract types。Adapter interface 优先使用 ref splitter、locator、format support check、parser/navigation primitives 等最小 building blocks；若实现证明过度细分且没有收益，再回到 design/spec/tasks 扩大到 `outline/read/find/info` operation handlers。
...
- Core 使用统一静态 adapter registry；adapter selection、adapter inspection 和 `doctor` 都以该 registry 作为 adapter implementation 候选事实源。
...
- 内部结构采用 `docnav-navigation` + `docnav-adapter-contracts`：前者调配 document operations，后者承载 adapter interface definitions；默认不新增 adapter support crate。
