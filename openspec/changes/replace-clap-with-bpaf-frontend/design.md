...
- [Risk] retained `clap` surface 与标准参数 metadata 之间出现职责重复 → Mitigation: 审计要求列出现有 help/default/possible-values owner，并只把 parser/help surface 作为呈现入口。
- [Risk] parser 的 typed parsing、fallback/default 或 required 语义泄漏到业务层 → Mitigation: integration 只允许使用 argv classification/frontend mapping 和 help surface；标准参数仍拥有默认值、required、range、enum 和 strict validation。
