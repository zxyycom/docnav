本 change 原计划用 `bpaf` 替换当前 clap-based argv frontend。经 `adopt-strict-input-boundaries` Track A 协调后，active artifacts 改为保留 `clap` 作为 strict core CLI parser/help owner，同时继续收缩 frontend 与 navigation input resolution 语义边界。

## Why

Docnav 的 document operation 参数语义由 navigation input resolution 和 selected adapter typed-field declarations 承接。strict core CLI contract 要求 parser/help surface 稳定、未知输入和 operation-inapplicable 输入在入口边界失败，并把参数语义留给 navigation input resolution owner。
...
- 参数语义、defaults、required、range、enum、operation applicability、source merge 和 strict validation 继续由 navigation input resolution 和 selected adapter typed-field declarations 负责。
- help 使用 `clap` surface，但数据来源必须是 command context、navigation parameter metadata 和 adapter native option metadata。
...
- 被当前 operation 消费的参数仍按 navigation input resolution 严格校验。
