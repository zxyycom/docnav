本 change 原计划用 `bpaf` 替换当前 clap-based argv frontend。经 `adopt-strict-input-boundaries` Track A 协调后，active artifacts 改为保留 `clap` 作为 strict direct CLI parser/help owner，同时继续收缩 frontend 与标准参数语义边界。

## Why

Docnav 的大部分 CLI 参数语义已经由标准参数流程承接。strict direct CLI contract 要求 parser/help surface 稳定、未知输入和 operation-inapplicable 输入在入口边界失败，并把参数语义留给标准参数 owner。
...
- 保留 `clap` 作为 direct CLI strict parser/help owner；本 change 不再引入 `bpaf` 替换 active direct CLI parser/help surface。
...
- unknown argv、extra positional 和 operation-inapplicable flag 在 strict direct CLI contract 下投影为 primary input diagnostic，而不是进入 adapter execution。
...
- 影响 `docnav-cli-args`、core CLI parser、adapter direct CLI args/help 和 CLI help/diagnostic tests。
- 不移除或降级 `clap` direct CLI parser/help 用途；若未来重新评估其它 parser 库，必须作为独立协调 change 处理。
