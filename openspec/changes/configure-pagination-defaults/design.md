...
- 不在本 change 中定义 protocol request 字段 shape。
...
本 change 以 `limit` 为目标字段名，实际 schema 和 example 迁移依赖 `structure-protocol-fields-and-readable-output`。

## Risks / Trade-offs

- 不暴露单位会降低跨 adapter 可比性，但保留 adapter-owned navigation 的弹性。
- disabled 归一依赖足够大的 numeric limit，仍需要测试避免分页循环或不可前进。
- 协议字段硬切换会影响 examples、schema、core、SDK 和 adapter fixture，需要跟随 `structure-protocol-fields-and-readable-output` 的字段决策细化任务。

## Open Questions

- 硬切换到 `limit` 后，哪些 CLI/help 文案需要同步改名？
