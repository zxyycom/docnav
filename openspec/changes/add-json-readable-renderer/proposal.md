本 proposal 将 JSON 专用 `readable-view` 定义为 `add-json-adapter` 之后的独立 handoff；当前只固定已确认的 owner 与边界，具体 presentation contract 尚未决定，因此本 change 仍处于 implementation-blocked 状态。
...
- **Current:** `docnav-json` 的 probe、outline、read、find、info 和 full-read 已走通现有 generic `readable-view` 与 `protocol-json`。当前代码、测试和 release artifact 只证明该 generic presentation；本 change 的 target 不是 Current。
...
- **Independent sequencing:** `redesign-token-cost-estimation`、`redesign-find-result-model`、`reuse-adapter-document-state` 和 `audit-runtime-performance-boundaries` 都不是本 change 的前置。它们的未决语义不得被复制或预选到本 change；若其中已落地的 Current contract 在实施前改变了输入事实，只在门禁审计中按新基线重核。
