...
Markdown adapter 的 token-informed cost 需要等待 `explore-structured-protocol-fields` 确认 raw protocol `cost` shape 后再落地到 schema、example 和 fixture。探索完成前，本 change 只保留 helper 和 adapter policy 的设计方向。
...
- `explore-structured-protocol-fields` 确认的 `cost` shape 是什么？
