...
- 连接失败 fallback 到旧 invoke，并输出诊断 warning。
...
2. service 默认启用，但保留 kill switch 和 fallback。
   - Rationale: 默认启用实现性能目标；连接失败 fallback 保留可用性和排障空间。
...
- [Risk] 默认 service 带来难排查的启动或连接问题 → Mitigation: kill switch、fallback warning 和 doctor/diagnostic path 必须进入审计。
...
- service diagnostics 是仅 stderr warning，还是增加 doctor/context 输出。
