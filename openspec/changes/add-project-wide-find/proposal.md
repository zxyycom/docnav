...
- Because the predecessors and this change fully modify some of the same owner requirements, predecessor completion is not enough by itself: this change must refresh its own overlapping `MODIFIED` bodies from the then-Current owners and preserve every already-effective predecessor clause. It never edits predecessor artifacts.
- `reuse-adapter-document-state` is not a predecessor and creates no implementation dependency. If it is Current when this change applies, project-find must preserve its navigation lifecycle clauses while rewriting overlapping complete requirements; if state reuse lands later, that change carries the symmetric rebase obligation.
