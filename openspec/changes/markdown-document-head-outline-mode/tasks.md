...
- [ ] 3.3 接入 adapter-owned native option sources `document_head_outline_mode` 和 `frontmatter_block_policy`，并让 navigation input resolution 合并 CLI/config sources 后传入 outline/find/read 语义；未声明 public input 必须被拒绝。
