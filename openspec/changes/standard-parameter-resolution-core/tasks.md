...
- [ ] 2B.4 实现 config source descriptor 和 load result：调用方提供 project/user config path、path origin 和 source level，标准参数层读取 JSON、校验顶层 object，并返回 loaded source 或 source-skipped diagnostic data。
...
- [ ] 2B.8 测试 source-skipped diagnostic handoff：标准参数层只返回 structured diagnostic data，不决定 stderr/stdout/exit code。
...
- [ ] 3.7 添加配置 source 读取 fixture，证明默认缺失 source 不返回 diagnostic，显式缺失、不可读、invalid JSON、non-object 返回 source-skipped diagnostic data 且继续合并其它来源。
