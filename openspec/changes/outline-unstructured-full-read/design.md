...
2. **非结构化全文读取不分页。**
   ...
   配置命中时，`page` 和 continuation 不出现在成功结果中；`limit_chars` 和 `page` 仍可由 CLI 参数解析层校验，以保持命令行兼容，但不用于裁剪全文内容。

   备选方案是按 read 的 `limit_chars` 分页。该方案更节省单次输出，但会重新引入 continuation 与多轮调用，违背“自动全文阅读”的用户意图。
...
- [Risk] 本 change 过早定义配置来源，后续可能与 adapter direct CLI 配置模型冲突。  
