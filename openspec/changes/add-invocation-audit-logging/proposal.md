...
Docnav 当前只有协议 `request_id` 和验证/烟测日志，缺少运行时调用记录；维护者难以在不重跑命令的情况下复盘一次核心 CLI 文档操作经过 adapter selection、navigation request construction、linked adapter handler dispatch、输出投影后的状态、耗时和失败边界。

需要先定义一个有限、安全、可审计的日志能力，避免后续为了调试临时把完整 document content、protocol payload、structured diagnostics 或 debug context 写入不稳定位置，破坏 stdout/stderr 契约或泄露敏感输入。

## What Changes

- 新增运行时调用日志能力，记录 `docnav` 文档操作到 linked adapter dispatch outcome 的元数据级事件，包括 `request_id`、operation、selected adapter id、路径显示策略、page/limit、耗时、operation/output status、响应大小摘要、错误分类和是否成功。
...
- Affected executable: `docnav` 核心 CLI 的文档操作执行路径，尤其是 adapter selection 后的 navigation request construction、linked adapter handler dispatch、结果校验和错误映射边界。
- Affected adapter surface: linked adapter handler 继续只返回结构化 result/diagnostic；首期日志由 core 记录 dispatch outcome，不要求 adapter 改变 handler payload、协议输出或自行写审计日志。
