...
- 新增运行时调用日志能力，记录 `docnav` 文档操作到 selected adapter dispatch outcome 的元数据级事件，包括 `request_id`、operation、selected adapter id、路径显示策略、page/limit、耗时、operation/output status、响应大小摘要、错误分类和是否成功。
...
- 无。本 change 创建新的运行时调用日志 capability；现有 `repository-quality-observability` 继续只拥有源码质量观测，`adapter-contract`、`protocol-contract`、`output-contract` 和 `core-cli` 的既有 requirement 不在提案阶段直接修改。
