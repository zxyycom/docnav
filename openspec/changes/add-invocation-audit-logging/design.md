...
Docnav 的核心文档操作会生成 protocol request，启动 adapter `invoke` 子进程，把请求写入 stdin，再从 stdout 读取 protocol response 并进行响应校验。当前这条链路只通过内存对象和 protocol `request_id` 关联；验证脚本和 smoke runner 会写 `.log/.../latest.log`，但这些是测试/验证运行日志，不是普通 CLI 调用的运行时审计记录。

本 change 影响 core CLI 到 adapter process 的运行时观测边界。它必须保持 adapter stdin/stdout/stderr 协议不变，也必须保持 `readable-view`、`readable-json` 和 `protocol-json` 的 stdout 纯净。
...
- 明确安全边界：默认不记录 document content、完整 protocol payload、完整 stderr、环境变量或 secrets。
- 明确验证边界：日志功能不能污染 stdout，不能改变 protocol response，不能让日志写入失败改变文档操作的成功/失败语义。

**Non-Goals:**

- 不改变 `RequestEnvelope`、`ProtocolResponse` 或 schema。
- 不把日志字段加入 `readable-json`、`protocol-json` 或 adapter protocol response。
- 不要求 adapter binary 自己实现长期日志系统。
...
备选方案是默认记录完整 payload。该方案调试方便，但会把 `read.content`、query、document path、stderr 或未来敏感字段固化到日志里，风险过高。
...
备选方案是立即引入日志库。该方案扩展性更好，但会提前扩大依赖和全局初始化面；如果首期只是 core `invoke` 调用记录，手写 writer 足以证明行为并降低引入风险。

### Decision 5: 日志写入失败不改变主调用结果

日志目录不可写、日志序列化失败或日志文件追加失败时，文档操作的成功/失败结果仍由原本 CLI/adapter 行为决定。实现可以在非 protocol stdout 的安全通道输出简短诊断，或在后续审计策略中选择静默降级；不得因为审计日志失败把成功的 document operation 改成失败。
...
- [Risk] trace 模式会记录文档正文或 adapter stderr。Mitigation: trace 必须显式开启，并在实现前加入 payload size cap、field redaction 和 negative tests。
...
3. 在 core CLI adapter `invoke` 边界实现 metadata-only JSONL event writer，并确保日志功能关闭或写入失败时不改变现有 CLI 行为。
4. 增加聚焦测试：stdout purity、metadata-only 默认、trace opt-in、截断/脱敏、日志写入失败降级和 request/response `request_id` 关联。
