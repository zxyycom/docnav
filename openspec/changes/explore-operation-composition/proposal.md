...
- 后续进入实现前，需要创建或更新更具体的 implementation change，届时再决定是扩展现有命令、增加 option、提供 SDK helper，还是新增极少数专用入口。
...
- 后续实现可能影响 `core-cli`、`docnav-contracts`、`adapter-sdk`、MCP mapping、readable output schema 和示例，但需要在后续具体 change 中重新定稿。
