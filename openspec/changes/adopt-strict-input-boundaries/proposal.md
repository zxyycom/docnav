...
- 标准参数层接收有明确 owner 的输入源；adapter-owned `options` 和 native options 作为 owner-scoped source 进入校验或拒绝流程。
- direct CLI 保留 `clap` 作为 parser/help 的默认实现依赖；严格输入契约由 parser/mapper 直接产出 input diagnostic。
...
- `core-cli`: direct CLI 输入边界按 strict-by-default 校验，并定义 explicit intent failure 的诊断规则。
...
- `adapter-protocol`: adapter direct CLI、`invoke` request、manifest/probe routing 与 adapter-owned options 使用严格输入责任边界。
...
- `markdown-navigation`: Markdown direct CLI、配置读取、adapter-owned options 和 smoke/矩阵测试使用 strict failure 与 primary `DiagnosticRecord` 证明。
...
- 受影响 Rust crate：`docnav`、`docnav-cli-args` 职责收缩、`docnav-standard-parameters`、`docnav-adapter-sdk`、`docnav-diagnostics`、`docnav-output`、adapter routing 模块，以及覆盖 strict failure、primary `DiagnosticRecord`、owner-scoped native options 和 success payload shape 的 smoke/unit tests。
- 受影响 public surface：direct CLI exit behavior、protocol/readable error shape、stdout/stderr channel policy、adapter selection semantics、config source handling、schema examples 和 test case expectations。
