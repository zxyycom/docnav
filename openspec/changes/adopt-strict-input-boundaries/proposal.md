...
Docnav 的 AI 友好性需要落到“失败后可一次修正”。调用方显式输入不符合契约时，CLI parser、navigation input resolution、adapter selection、输出模式、schema 和测试围绕同一个 owner 边界返回可修复错误，长期维护成本更低。
...
- `navigation-input-resolution`: direct input、config source 和 operation applicability 使用 owner-scoped source 规则；adapter native options 是 selected adapter typed-field declarations 声明的明确 owner-scoped 输入源。
- `adapter-protocol`: linked adapter handler boundary、protocol request source handling、manifest/probe/descriptor metadata 与 adapter-owned options 使用严格输入责任边界。
...
- `markdown-navigation`: core-linked Markdown adapter、navigation config source loading、adapter-owned options 和 core CLI smoke/矩阵测试使用 strict failure 与 primary `DiagnosticRecord` 证明。
...
- OpenSpec spec deltas 覆盖 `docnav-contracts`、`core-cli`、`adapter-protocol`、`navigation-input-resolution`、`readable-view-output` 和 `markdown-navigation` 中的 strict input boundary、primary `DiagnosticRecord`、owner-scoped native options、success payload projection 和 readable conformance 契约。
