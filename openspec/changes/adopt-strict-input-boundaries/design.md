...
这些策略原本服务 AI 调用体验：尽量让一次命令在核心语义足够时成功。新的目标是把 AI 友好性放到可修复失败上，让 parser、standard parameter resolution、linked adapter contract、diagnostics、output、schema 和测试围绕同一条 strict contract 收敛。
...
5. 并行轨道 D：实现 crate 切片，按 core CLI/adapter selection、standard parameters/config/native options、diagnostics/output、linked adapter contract 和 adapter-side validation 分工。
