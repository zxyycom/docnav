...
Docnav 需要把公共输入问题集中到 owner 边界处理：direct CLI 识别 unknown argv、extra positional 和 operation-inapplicable known flag；`docnav-navigation` 区分显式 adapter intent 与内部 discovery；navigation config source loading 区分 absence 与 invalid state；readable 输出和 schema 按成功 payload 或 failure diagnostic 分别投影。

这些策略原本服务 AI 调用体验：尽量让一次命令在核心语义足够时成功。新的目标是把 AI 友好性放到可修复失败上，让 parser、navigation input resolution、linked adapter contract、diagnostic helper boundary、output、schema 和测试围绕同一条 strict contract 收敛。
...
- 将 core CLI argv、protocol request arguments、配置文件、显式 adapter/ref/path/operation 参数等公共输入边界改为 strict-by-default。
...
- 保留 `clap` 作为 strict core CLI parser/help 的默认依赖；strict parser/mapper 负责把 invalid argv 转成 input diagnostic。
...
AI 友好性通过诊断质量实现。公共失败面使用一个 primary `DiagnosticRecord`。`docnav-diagnostics` 作为 diagnostic/error model helper crate，提供 stable identity、record construction/validation 和 details helper materials；protocol、output、CLI、adapter 等 surface owner 负责各自 public projection、framing、channel 和 exit behavior。此前独立错误通道 owner 的职责已收敛到 helper boundary 和 surface owner，不再恢复为单独 owner 文档。最小结构为：
...
### Decision 6: Strict core CLI keeps clap as parser/help owner

strict 模式下 core CLI 保留 `clap` 作为 parser/help owner 的实现依赖。unknown flags、extra positional、missing value 和 invalid value 由 strict parser 或入口校验转换为统一 input diagnostic。共享 argv helper 的职责收缩为 strict parser/mapper 需要的能力。
...
3. 并行轨道 B：更新 owner docs，按 `docs/navigation.md` 的 owner 表拆成 architecture/CLI、navigation input resolution/adapter/protocol、diagnostic helper boundary/output、testing/navigation/Markdown。
4. 并行轨道 C：更新 schema/examples/fixtures，表达成功 payload shape 和 failure primary `DiagnosticRecord` projection。
5. 并行轨道 D：实现 crate 切片，按 core CLI/adapter selection、navigation input resolution/config/native options、diagnostic helper/output、linked adapter contract 和 selected adapter typed-field validation 分工。
...
无未回答开放问题。已收敛：Decision 2 已记录 diagnostic helper boundary 与 surface projection owner 的分工，错误通道不再作为单独 public owner 维护。Change 已准备进入实现阶段；active changes 的协调工作由 Track A 在相关实现切片前完成。
