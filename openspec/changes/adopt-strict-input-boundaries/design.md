...
这些策略原本服务 AI 调用体验：尽量让一次命令在核心语义足够时成功。新的目标是把 AI 友好性放到可修复失败上，让 parser、navigation input resolution、linked adapter contract、diagnostics、output、schema 和测试围绕同一条 strict contract 收敛。
...
3. 并行轨道 B：更新 owner docs，按 `docs/navigation.md` 的 owner 表拆成 architecture/CLI、navigation input resolution/adapter/protocol、diagnostics/output、testing/navigation/Markdown。
4. 并行轨道 C：更新 schema/examples/fixtures，表达成功 payload shape 和 failure primary `DiagnosticRecord` projection。
5. 并行轨道 D：实现 crate 切片，按 core CLI/adapter selection、navigation input resolution/config/native options、diagnostics/output、linked adapter contract 和 selected adapter typed-field validation 分工。
