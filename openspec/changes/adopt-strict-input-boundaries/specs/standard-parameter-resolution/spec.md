...
- **WHEN** adapter direct CLI、config 或 invoke input 包含已声明的 adapter native option
...
- **WHEN** adapter direct CLI、config 或 invoke input 包含 undeclared native option
...
- **WHEN** caller 收到 CLI argv tokens 或 decoded adapter `invoke` JSON value
