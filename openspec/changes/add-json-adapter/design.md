...
| A5c | [自定义渲染边界](../../../docs/decisions/navigation-output/keep-custom-rendering-in-readable-view.md) | 当前 change 用 generic `readable-view` 走通每个 operation 并记录格式假设；后续 change 基于稳定 raw facts 确定 JSON 信息密度、层级、标点、preview、分页显示与 renderer mechanics | **SEQUENCED** — 由任务 6.4 建立后续 change |
...
格式专用自定义渲染需要同时确定信息密度、层级、标点、preview、分页显示和 renderer 选择 mechanics，因此由任务 6.4 建立的专门 change 承接。该阶段不是可选美化；完成它之后，JSON adapter 才形成覆盖完整行为的边界验证证据。
