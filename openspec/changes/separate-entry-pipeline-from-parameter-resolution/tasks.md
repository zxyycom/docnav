本 tasks 已按 `adopt-core-linked-adapter-libraries` 更新，范围是 core entry pipeline、parameter source resolution 和 static registry implementation source boundary。

## 1. Blocking Audit

- [ ] 1.1 确认 proposal、design、specs 和 tasks 都围绕“core entry pipeline + parameter source resolution + static registry implementation source boundary + raw input immutable”。
...
- [ ] 3.1 在 core CLI 中调整内部命名，使 document operation 入口体现 entry pipeline 和 parameter source resolution 调用边界。
- [ ] 3.2 在 `docnav-navigation` 中保持 request construction/adapter dispatch 与 parameter source resolution 分离。
- [ ] 3.3 明确配置来源合并通道实现边界：只负责 project/user config source loading、skip diagnostics、registered config path projection 和 source contribution。
...
- [ ] 5.1 运行相关 Rust tests，覆盖 core CLI parser/config、navigation request construction 和 parameter source resolution。
