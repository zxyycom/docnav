...
- 将“配置来源合并通道”定义为入口参数来源解析的子流程：它只负责 project/user config source 读取、跳过诊断、字段投影和来源参与。
...
- `standard-parameter-resolution`: 将能力语义从“标准参数解析”改为“入口参数来源解析”，保留字段身份、来源优先级、typed validation、passthrough 和 operation argument binding，并限制其只处理入口提供的 input view 与来源对象。
...
- Affected public surfaces: CLI help wording, docs terminology, warning/error ownership descriptions, schema/example validation references, and adapter SDK contract documentation.
- Compatibility expectation: runtime behavior should remain compatible. If a name is externally observable in docs, test fixtures, diagnostics, crate/module paths, or generated artifacts, the implementation must either provide a compatibility alias or document the migration and update validation material in the same work item.
