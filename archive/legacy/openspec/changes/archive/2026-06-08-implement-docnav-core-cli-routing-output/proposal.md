...
- 实现 adapter 选择流程：
  ...
  5. 预选 adapter 或 registry 遍历中的候选 manifest/probe 输出不符合当前 schema 或语义校验时，记录候选证据并继续遍历；后续候选成功时按输出模式呈现 warning，全部候选失败时集中报告 `FORMAT_UNKNOWN` candidates；选中 adapter 的 invoke 输出不符合当前 schema 或语义校验时直接失败。
