...
**适用状态：本文定义 `add-json-adapter` 的目标，不表示 Current 能力；当前支持仍以 `docs/`、代码和测试为准。**

## Why

当前静态 linked-adapter 架构只有 Markdown 一个真实实现，单一样本不足以区分共享 contract 与 Markdown-specific implementation。JSON 简单、常用、结构明确，使用 object key、array index 和 value tree，能够以可控范围检验 `probe/selection -> outline|find -> ref -> read`、`info`、full-read、registry 和 release path。推进依据是架构边界验证。
...
- JSON outline 按 object member 源码顺序和 array index 做确定性树前序；number identity 使用原始 token；structured read 使用同一 member 顺序、pinned serializer 的自然结果和两空格布局；find 直接搜索原文并把源码命中映射为可继续 read 的 node ref。
...
- 将 Decision Map 中的跨格式活动决策同步到 `docs/architecture.md`、`docs/adapter-contract.md` 和 `docs/output.md`，将 JSON-specific 行为同步到 JSON adapter 主规范。
...
- 当前 change 记录 generic `readable-view` 暴露的格式假设；任务 6.4 据此建立专门的 JSON 自定义渲染 change，继续检验 presentation boundary。
