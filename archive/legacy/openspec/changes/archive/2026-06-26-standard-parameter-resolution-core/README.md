...
调用方继续用 `docnav-typed-fields` 声明字段 identity、类型、required/default、range、enum、regex、各 mapped value processing strategy path 和 passthrough processing build。typed-fields 的 `process` 返回字段 extraction result 和 caller processing result；标准参数层只消费 metadata 和 caller passthrough processing result，并负责 config loading、source construction、来源合并、typed-field validation、diagnostic events 和 passthrough handoff。
