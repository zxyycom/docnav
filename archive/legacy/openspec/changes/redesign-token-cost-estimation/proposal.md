...
- `add-json-adapter` 已归档，Current JSON owner 是 `docs/adapters/json.md` 与 main `json-adapter` capability。任务 1 的决策门禁必须按最终批准的 cost contract 判断是否需要在本 change 增加完整 `json-adapter` delta；本轮状态修正不预先作出该范围或兼容性决定，也不修改归档 change。
...
- `redesign-find-result-model` 与本 change 会完整修改部分同名 owner requirements。两者仍无实现依赖；后落地者必须在自己的 change 目录按当时 Current owner 重写重叠的完整 `MODIFIED` requirement，并保留先落地 change 已生效的条款。
