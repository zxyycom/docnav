...
多条相关失败只能作为 primary record 的从属结构化 details 暴露，例如 `field_issues`、`config_issues`、`typed_validation_failures` 或 `candidate_failures`。内部调试记录和 automatic discovery 中的 candidate failures 可以继续压入同一通道；public failure contract 仍只由一个 primary record 驱动。

一条记录至少表达这些语义：

- 发生了什么问题。
- 问题属于 error 还是 fatal 等级别。
...
每个 top-level `docnav` command 或 protocol request 都有自己的错误栈。栈的生命周期不跨独立请求。
...
Docnav runtime 和 public surface 上发现的问题都进入错误通道，包括核心 document operation、protocol request construction、adapter layer dispatch、标准参数处理、adapter 选择、配置命令、初始化、doctor、version 和 help 等路径。

写入通道的模块只描述自己看到的问题和影响，不负责替外层组织最终失败。Automatic discovery 中一个 adapter 候选失败时可以记录为 discovery state 并让路由继续尝试其它候选；若后续候选成功，这些记录不进入 public success output；若全部候选失败，它们只能作为 primary failure 的从属 details 暴露。显式 public input 失败必须在 owning boundary 形成 blocking diagnostic，不能作为可恢复问题继续执行。
...
- [适配器契约](adapter-contract.md) 定义 adapter library interface、manifest metadata、probe 行为、声明式选择、自动发现和候选失败边界。
...
- Public failure surface 只投影一个 primary record；相关 field/config/typed/candidate failures 保持 subordinate details。
