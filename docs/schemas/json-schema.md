...
本目录使用 JSON Schema Draft 2020-12，并按语义层拆分。Schema 是字段形状、surface projection 和示例的校验材料，不是新的规范来源；字段语义、诊断 code/details 规则、输出承载和职责边界由对应主规范文档定义。
...
operation readable schema 包含可省略的顶层 `warnings` 数组。warning 的来源、承载位置和稳定字段由 [输出模式](../output.md) 定义，机械身份和 details 规则来自 [错误通道](../diagnostics.md) 的 projection；本文件只维护 schema `$defs` 与示例校验入口。
...
配置 schema 只描述文档化的 JSON 文件形状，可用于示例校验和编辑器提示。配置文件发现、字段映射、来源合并、未映射字段处理、错误归属和 adapter-specific 字段语义由 [标准参数](../standard-parameters.md)、[适配器契约](../adapter-contract.md) 和对应 adapter 文档拥有。
...
`docnav-json-io` 拥有低层 serialization、newline writing 和 write failure plumbing。protocol request/response、manifest、probe 和 readable schema 的字段 shape 仍由本目录维护；语义校验、错误归属、诊断 projection 和通道承载由对应 owner 文档与实现测试验收。
