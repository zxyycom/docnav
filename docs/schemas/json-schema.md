...
| Schema | 用途 |
| --- | --- |
| [docnav-markdown-config.schema.json](docnav-markdown-config.schema.json) | `docnav` 配置 source 中 Markdown native option、document operation defaults、navigation-owned outline selector 和 core-owned `invocation_log` section 的字段形状和示例校验；selected adapter declarations 的参数语义、selector priority、path matching、threshold comparison 和 invocation logging enablement 不由 config schema 拥有 |
