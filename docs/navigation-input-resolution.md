...
5. Direct/native command input 由 navigation-private source adapter 遍历同一 `FieldDefSet` 的 canonical processing metadata 后产出 explicit source；该 adapter 只映射已经解析的 input，不维护平行 locator 或字段事实。Project/user JSON 通过 config companion 按同一 `FieldDefSet` 的 config-path metadata 提取 declared candidates。相同 metadata 同时服务 strict config validation、`docnav config inspect` source facts 和 navigation config value validation。
...
| Projection | 消费方 | 边界 |
| --- | --- | --- |
| CLI/input projection | CLI argv/direct input 和 navigation operation input | 描述 direct input flag、operation binding、typed validation 和 `OperationArguments` / `NativeOptionHandoff` binding。 |
...
docnav-navigation
  ...
  build one canonical FieldDefSet from common fields + selected adapter declarations
  map direct input and extract declared project/user config candidates
