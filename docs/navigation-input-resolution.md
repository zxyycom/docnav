...
| 来源 | 提供方 | 用途 |
| --- | --- | --- |
| explicit | fixed command facts + normalized CLI `Source` | 本次调用显式传入的 operation、path、ref/query，以及保留 canonical identity 的 page、limit、output、auto_read、adapter intent 和 adapter-scoped parameter candidates |
...
| built_in | core catalog static defaults | 缺省 pagination、limit、output、auto-read、page 和 adapter-scoped 参数默认值 |
...
| `defaults.auto_read` | `outline` / `find` 的 auto-read mode，只能是 `disabled` 或 `unique-ref`。 |
...
`defaults.auto_read` 是 full config shape 中的 known field，因此 valid value 在 `read` / `info` config source 中也能通过 full validation；它只在 `outline` / `find` selected-operation view 中参与 resolution。非法 enum value 使用现有 source-attributed typed validation diagnostic，并在 adapter dispatch 前失败。
...
Auto-read entry 的 canonical identity 是 `docnav.defaults.auto_read`，类型是 allowed values 恰好为 `disabled` / `unique-ref` 的 string enum。它声明 CLI locator `--auto-read`、config locator `defaults.auto_read`、built-in default `unique-ref`、merge strategy `Replace`、operation bindings `outline` / `find` 和 core navigation orchestration consumer；不声明 environment locator。其来源顺序因此固定为 `explicit > project > user > built_in`，同名或相似 environment variable 不产生 candidate。
...
Resolved mode 为 `disabled` 时，navigation 只执行请求的 base outline/find operation。Resolved mode 为 `unique-ref` 时，navigation 在 base response 成功并通过 protocol 校验后，仅检查当前返回结果：structured outline 使用 `entries[].ref`，find 使用 `matches[].ref`；对非空 ref 做 string-exact 去重，distinct 集合恰好为一个时才尝试一次 read。
