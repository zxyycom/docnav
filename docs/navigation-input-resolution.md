...
5. Direct command input 由 navigation mapping 产出 explicit source；project/user JSON 通过 config companion 按同一 `FieldDefSet` 的 config-path metadata 提取 declared candidates。相同 metadata 同时服务 strict config validation、`docnav config inspect` source facts 和 navigation config value validation。
