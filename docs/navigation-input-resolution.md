...
5. 从同一批 owner-provided metadata 产出 CLI/input projection 和 config-source projection。CLI/input projection 服务 direct argv 与 operation input；config-source projection 服务 config source validation、`docnav config inspect` source facts 和 navigation config value validation。
6. 将 explicit command value、project config、user config 和 built-in default 作为来源集合，按 `explicit > project > user > built_in` 解析每个声明参数。
7. 使用 typed-field 的底层校验和提取函数得到 typed 参数；解析失败时返回带来源信息的 blocking diagnostic。
...
Parameter aggregation 是 navigation/config 输入边界的一层 metadata projection，不是新的 owner 或 parallel aggregation layer。各 owner 继续声明自己的字段事实：
...
- `docnav-navigation` 声明通用 document operation fields、outline selector config fields、source priority、config source loading、selected adapter/operation resolution 和 request construction。
...
- `docnav-typed-fields` 提供 `FieldDefSet`、processing metadata lookup、candidate value validation 和 canonical typed value facts；它不拥有 source priority、CLI flag syntax、adapter selection、public diagnostic code 或 protocol/readable envelope。
