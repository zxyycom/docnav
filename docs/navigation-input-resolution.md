...
4. 对 navigation 命令，把 operation、固定 positional/path facts、normalized document CLI `Source`、config source descriptors/paths 和当前 core release 的 adapter registry 交给 `docnav-navigation`。
...
4. 构造 canonical operation `FieldDefSet`：注册通用 operation 参数，并从 selected adapter definition 注册当前 operation 适用的 `AdapterOptionSpec` declaration，保留 adapter-owned native options、内置默认值、processing locator 和 handler binding metadata。
5. Fixed positional input 由 navigation-private direct mapping 产出；core 提供的 normalized CLI `Source` 已保留 canonical field identity、locator、typed/invalid input 和 source attribution。Project/user JSON 通过 config companion 按同一 `FieldDefSet` 的 config-path metadata 提取 declared candidates。相同 metadata 同时服务 strict config validation、`docnav config inspect` source facts 和 navigation config value validation。
6. 将 canonical `FieldDefSet` 与 explicit、project、user sources 交给 resolution core；static default 由 field metadata 自动回退，并按 `explicit > project > user > built_in` 解析每个声明参数。
...
8. 构造 `RequestEnvelope` / `OperationArguments` 和 handler-facing selected adapter `NativeOptionHandoff`，并通过 selected adapter definition 调用对应 operation handler。
...
**Current：** navigation 在 argv parsing 前从 applicable common declarations、registry order 和各 adapter declaration order 构造 operation-scoped registry CLI `FieldDefSet`；core 交回 fixed positional/path facts、config descriptors/paths、registry 和 normalized typed/invalid CLI `Source`。Navigation 先从 routing declarations 解析 adapter intent 并完成 selection，再从同一 declarations 构造 common + selected adapter/current-operation `FieldDefSet`。只有 selected-set member 进入 priority、merge、validation 与 materialization；任何不属于该 set 的 explicit candidate 都在 request construction/dispatch 前严格失败。其它已知 adapter config namespace 继续遵循 config-source contract。

Registry set 与 selected set 是 canonical `FieldDefSet` 的不同 operation/selection projection，不是两套参数模型。Field-set parity、selected/unselected candidate behavior、request construction 与 typed handoff 由 navigation field-set/input-resolution/hard-cutover tests 证明；完整 runtime 调用关系见 [架构](architecture.md#document-named-option-派生状态)。
...
| 来源 | 提供方 | 用途 |
| --- | --- | --- |
| explicit | fixed command facts + normalized CLI `Source` | 本次调用显式传入的 operation、path、ref/query，以及保留 canonical identity 的 page、limit、output、adapter intent 和 native option candidates |
...
| `options.<adapter-id>.<option-key>` | Adapter-owned native option source value。`<adapter-id>` 是 static registry adapter id，`<option-key>` 是 adapter declaration 提供的 native option key。 |
...
Parameter aggregation 是 navigation/config 输入边界对 canonical `FieldDefSet` 的消费关系。各 owner 继续声明自己的字段事实：

- core CLI 声明 core-owned runtime config field facts，例如 `invocation_log.*`；CLI 仍拥有 config command surface、argv lexical parsing 和非 navigation command 行为。
- `docnav-navigation` 声明通用 document operation fields、outline selector config fields、source priority、strict config shape/applicability validation、selected adapter/operation resolution 和 request construction。
- Registered adapter definition 通过 `AdapterOptionSpec` 声明 native option facts：owner、option key、operation applicability、value kind、constraints、defaults、source binding 和 handler binding metadata。
- `docnav-typed-fields` 提供唯一的 `FieldDef` / `FieldDefSet` 参数模型、processing locator、merge strategy、candidate/final value validation 和 canonical typed value facts；它不拥有 source priority、CLI command structure、adapter selection、public diagnostic code 或 protocol/readable envelope。
- `cli-config-resolution` 直接消费该 `FieldDefSet` 和 normalized sources，执行优先级、merge、default fallback、provenance、diagnostics 和 all-or-nothing materialization；它不复制 field identity、validation 或 default metadata。
- `cli-config-resolution-serde` 按 config-path processing metadata 提取 declared project/user candidates；额外 config 字段的 strict rejection 和 operation applicability 继续由 `docnav-navigation` 负责。

同一批 owner-provided facts 必须能产出两个 projection：

| Projection | 消费方 | 边界 |
| --- | --- | --- |
| CLI/input projection | Core CLI、lexical/preflight 和 navigation operation input | Registry projection 描述 argument identity、flag、help/value name、capture cardinality 和 typed/invalid candidate；selected projection 描述 operation applicability、typed validation 和 `OperationArguments` / `NativeOptionHandoff` binding。 |
| Config-source projection | config source validation、`docnav config inspect` 和 navigation config value validation | 描述 JSON config source path、owner、adapter id when applicable、field identity、value kind、constraints、defaults 和 owner-specific shape validation handoff。 |

Config-source projection 从 registry-backed adapter declarations 产出 adapter native option path `options.<adapter-id>.<option-key>`。同名 `<option-key>` 在不同 `<adapter-id>` namespace 下保持独立；projection 不把它折叠为 core-owned 全局字段。`docnav config inspect` 可以消费该 projection 展示 source facts，但 selected adapter/operation 是否消费某个值仍由 navigation input resolution 的 selected operation 阶段决定。

**Current：** config-source projection、operation-scoped registry CLI set、common/native field CLI metadata、normalized candidate handoff、selected operation `FieldDefSet` 和 selected-set identity membership 均已进入 runtime。Fixed positional 继续使用 navigation-private direct processing；named options 只通过 field-derived CLI projection 进入 normalized source。
...
docnav core
  ...
  if navigation: build static/generated command from registry CLI field set
  extract normalized typed/invalid CLI Source
  pass fixed command facts + CLI Source + config source descriptors/paths + registry

docnav-navigation
  load raw project/user config sources
  validate source shape/keys through config-source projection and owner-specific shape validation
  parse routing-required input
  select adapter from registry
  build selected FieldDefSet from common fields + selected adapter declarations
  reject explicit candidates outside the selected/current-operation set
  map fixed positional input and extract declared project/user config candidates
  resolve sources with explicit > project > user > built_in static defaults
...
1. Source validation 阶段读取 project/user config source，使用 config-source projection 和 owner-specific shape validation 报告 unknown field、unknown adapter id、nested shape failure、declared scalar typed value failure 和 adapter-local declaration conflict。Known adapter namespace 下的 source facts 可以作为 source facts 保留。
2. Selected adapter/operation 阶段只消费 selected adapter id namespace 中当前 operation 声明接收的 native option values。其它已知 adapter namespace 不用于 selected operation argument construction，不 forward 给 selected adapter handler；selected namespace 下未声明给该 operation 的 option 是 blocking unsupported native option diagnostic。
...
当 `pagination.enabled` 最终为 `false` 时，resolution 在构造 operation arguments 前把分页预算归一为最大正整数预算。该归一化不回写 fixed command facts、normalized CLI source、config source 或 protocol JSON。
...
- 参数 identity、owner、`options.<key>` final arguments path、`options.<adapter-id>.<option-key>` config-source path 和 operation applicability。
...
`OperationArguments.options` 保持 adapter-owned request namespace，不改变 protocol/readable 或 handler-facing payload。多个 adapter 或多个 type variant 可以声明同名 option key；resolution 不把同名 key 折叠为 core-owned 字段。Persistent config source namespace 使用 `options.<adapter-id>.<option-key>`；adapter id 直接来自 registry id，不使用 alias。Adapter selection 完成后，只解析 selected adapter definition 声明接收的 native option sources。未被 selected definition 声明接收的 selected-adapter `options.<adapter-id>.*` source 必须严格失败，不进入 dispatch；其它已知 adapter id namespace 保持为独立 source facts，不 forward 给 selected adapter handler。

Canonical `FieldDefSet` 负责字段级校验和 typed materialization。Adapter 可以通过 declaration 表达格式语义约束，但 handler 不再接收未校验 raw option value 来执行基础类型或范围校验。`docnav-navigation` 只组合通用字段与 selected adapter declaration、按 selected field identity 过滤 normalized CLI source、映射 fixed positional input、调用 resolver，并完成 `OperationArguments` 绑定和 `NativeOptionHandoff` 构造；它不从 config key、CLI flag 或独立 registry entry 重新推导 adapter-owned locator、value kind、range、merge 或 default 语义。Config unknown-option detection 使用同一 canonical set 的 config-path processing metadata 和当前 operation applicability 判断哪些 `options.<adapter-id>.*` source 已被声明接收。
