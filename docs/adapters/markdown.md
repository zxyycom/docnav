...
以下 caller-configurable 默认值属于 core parameter catalog；来源解析、selected-operation filtering、closed input construction 和 request construction 见 [Navigation Input Resolution](../navigation-input-resolution.md)。Markdown 本页记录格式行为所依赖的默认值、operation 适用范围和语义：
