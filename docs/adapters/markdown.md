...
共享协议、`docnav` core 和 MCP 只按共享 ref 契约原样传递 ref，不解析 Markdown heading ref 内部结构。共享 ref 最小契约见 [Ref](../ref-contract.md)。
...
- 字符预算允许时使用 `...` 作为显式截断标记。
- 当完整 ref 已耗尽或几乎耗尽预算时，display 可压缩为最小非空文本，不要求额外截断标记。
- ref 保持完整且不受影响。
