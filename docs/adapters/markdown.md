...
Markdown adapter 内置 `max_heading_level: 3`（可由 `docnav-markdown` 配置域覆盖）。可见性过滤决定 outline 返回哪些 heading entries；可见 heading 的 line/level 结构坐标保持稳定，保证同一解析结果中的同一 heading 在 outline 和 find 中使用相同 ref。
