...
Outline entry 的 `label` 承载 heading title；`metadata.heading_level` 承载 heading level；`cost.measurements[]` 承载 section 的 `lines`、`bytes`、`tokens` 成本。ref 承载 line 和 level 结构坐标。
