...
Markdown adapter 的静态 descriptor 声明 `options.max_heading_level` native option registry entry，内置默认值为 `3`，并暴露 `--max-heading-level` CLI native option source。Core 标准参数流程可从 CLI/config/protocol `arguments.options` 中取得并合并该 option 的最终 source value，但不按 Markdown 语义预校验；Markdown handler 在消费时校验该值必须是 `1..6` 的整数，并返回 adapter-owned structured diagnostic 表达 type mismatch 或 range invalid。可见性过滤决定 outline 返回哪些 heading entries。可见 heading 的 line/level 结构坐标保持稳定，保证同一解析结果中的同一 heading 在 outline 和 find 中使用相同 ref。
...
以下默认值属于 core-linked `docnav-markdown` 静态 descriptor 和标准参数声明；配置字段映射、来源合并、native option handoff 和 core request construction 策略由 [标准参数](../standard-parameters.md) 定义。Markdown 本页记录格式私有默认值、operation 适用范围和 adapter-side option semantics：
...
Protocol request `arguments` 表达当前 document operation 的显式输入；省略的已注册标准参数可由 core 配置或静态 descriptor 默认值补足。分页操作的 page 省略时固定从 `1` 开始。
...
Markdown native option 的 core 配置参考形状见 [docnav-markdown-config.schema.json](../schemas/docnav-markdown-config.schema.json) 和 [docnav-markdown-config.json](../examples/json/docnav-markdown-config.json)。这些文件只作为配置填写提示和示例校验；配置发现、字段映射、来源合并、失败处理和 runtime 参数校验由 [标准参数](../standard-parameters.md) 与 [适配器契约](../adapter-contract.md#标准参数消费边界) 定义，runtime 不要求先加载该 schema。Config 层只做全局 key/source/shape 处理或保留 raw value，`max_heading_level` 的 `1..6` 语义由 Markdown adapter 消费时校验。

`options.max_heading_level` 是 Markdown adapter-owned native option source。Core CLI/config 或 protocol request `arguments.options` 通过源码级 native option registry 进入 generic merge；adapter selection 后，core 只把 selected Markdown descriptor 支持的 options 投影给 Markdown handler。Markdown handler 消费时只接受 outline/find 适用的 `max_heading_level`，并将 type mismatch 或 `1..6` 范围外值返回为 adapter-owned structured diagnostic。
