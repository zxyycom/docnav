...
Markdown adapter 的静态 descriptor 声明 `options.max_heading_level` adapter-owned typed-field declaration 和内置默认值 `3`，并暴露 `--max-heading-level` CLI native option source。Adapter selection 后，`docnav-navigation` 将 selected Markdown `AdapterOptionSpec` 注册进 operation field set，按来源优先级解析该 option，并通过 typed-field helper 校验/提取为 `1..6` 的整数。可见性过滤决定 outline 返回哪些 heading entries。可见 heading 的 line/level 结构坐标保持稳定，保证同一解析结果中的同一 heading 在 outline 和 find 中使用相同 ref。
...
以下默认值属于 core-linked `docnav-markdown` 静态 descriptor 和 adapter-owned typed-field declarations；来源解析、native option handoff、selected adapter declaration registration 和 request construction 策略见 [Navigation Input Resolution](../navigation-input-resolution.md)。Markdown 本页记录格式私有默认值、operation 适用范围和 adapter-owned option semantics：
