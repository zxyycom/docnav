...
`options.max_heading_level` 是 Markdown adapter-owned native option source。CLI/config source 通过 Markdown definition 中的 `AdapterOptionSpec` declaration 进入 navigation input resolution；adapter selection 后，`docnav-navigation` 只解析 selected Markdown definition 注册出的 options。Type mismatch 或 `1..6` 范围外值在 typed-field validation/extraction 阶段返回带来源的 diagnostic。
