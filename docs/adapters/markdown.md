...
Markdown adapter 的静态 descriptor 声明 `options.max_heading_level` native option registry entry，内置默认值为 `3`。Core 标准参数流程可从 CLI/config/protocol `arguments.options` 中取得并合并该 option 的最终 source value，但不按 Markdown 语义预校验；Markdown handler 在消费时校验该值必须是 `1..6` 的整数，并返回 adapter-owned structured diagnostic 表达 unsupported option、type mismatch 或 range invalid。可见性过滤决定 outline 返回哪些 heading entries。可见 heading 的 line/level 结构坐标保持稳定，保证同一解析结果中的同一 heading 在 outline 和 find 中使用相同 ref。
...
`options.max_heading_level` 是 Markdown adapter-owned native option source。Core CLI/config 或 protocol request `arguments.options` 通过源码级 native option registry 进入 generic merge；core 不因 selected adapter 不支持某 option、类型不匹配或范围非法而提前拒绝。Markdown handler 消费时只接受 outline/find 适用的 `max_heading_level`，并将 unsupported option、type mismatch 或 `1..6` 范围外值返回为 adapter-owned structured diagnostic。
