...
`options.max_heading_level` 是 Markdown adapter-owned native option source。Adapter direct CLI、adapter `invoke` `arguments.options` 和 adapter 配置 `options` object 只有在 registration 声明该 option 时才可传入；未知 option key、operation 不适用 option 或非法 option value 必须在 handler execution 前返回输入诊断。
...
- 默认 `pagination.enabled`、`limit`、`max_heading_level` 和 page 从 `1` 开始的分页规则。
- `docnav-markdown` direct CLI 配置优先级、路径覆盖、default config absence、invalid config failure、`defaults.output` 和 `options.max_heading_level` 对 outline/find 的可观察行为。
