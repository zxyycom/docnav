...
Markdown adapter 的静态 descriptor 声明 `options.max_heading_level` native option registry entry、typed-field 参数声明和内置默认值 `3`，并暴露 `--max-heading-level` CLI native option source。Adapter selection 后，`docnav-navigation` 只读取 selected Markdown descriptor 的 typed-field 参数声明，按来源优先级解析该 option，并通过 typed-field helper 校验/提取为 `1..6` 的整数。可见性过滤决定 outline 返回哪些 heading entries。可见 heading 的 line/level 结构坐标保持稳定，保证同一解析结果中的同一 heading 在 outline 和 find 中使用相同 ref。
...
以下默认值属于 core-linked `docnav-markdown` 静态 descriptor 和 selected adapter typed-field 参数声明；来源解析、native option handoff 和 request construction 策略见 [Navigation Input Resolution](../navigation-input-resolution.md)。Markdown 本页记录格式私有默认值、operation 适用范围和 adapter-owned option semantics：
...
Markdown native option 的 core 配置参考形状见 [docnav-markdown-config.schema.json](../schemas/docnav-markdown-config.schema.json) 和 [docnav-markdown-config.json](../examples/json/docnav-markdown-config.json)。这些文件只作为配置填写提示和示例校验；配置发现、字段映射、来源合并、失败处理和 runtime 参数校验由 [Navigation Input Resolution](../navigation-input-resolution.md) 与 [适配器契约](../adapter-contract.md#文档操作执行边界) 定义，runtime 不要求先加载该 schema。Config source 只提供 raw source value，`max_heading_level` 的 `1..6` 语义由 Markdown typed-field 参数声明表达，并在 request construction 前校验/提取。

`options.max_heading_level` 是 Markdown adapter-owned native option source。CLI/config source 通过源码级 native option registry 和 selected adapter typed-field 声明进入 navigation input resolution；adapter selection 后，`docnav-navigation` 只解析 selected Markdown descriptor 支持的 options。Type mismatch 或 `1..6` 范围外值在 typed-field validation/extraction 阶段返回带来源的 diagnostic。
...
- Core CLI config source descriptor/path handoff、path context、navigation-owned default config absence、invalid config failure、`defaults.output`、navigation input resolution native option handoff 和 Markdown adapter 对 `options.max_heading_level` 的 outline/find 可观察行为。
