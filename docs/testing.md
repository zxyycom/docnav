...
- warning/error 投影的承载位置、stdout/stderr 边界和 schema 校验。
...
- diagnostics stack/id/mark/order、DiagnosticCode details/投影、direct CLI argv compatibility、document output orchestration、低层 JSON writer 和 paging helper 的可观察行为边界。
...
4. schema、示例和 fixture 只校验 protocol/readable 输出投影，不成为新的业务语义或 code/details 规则来源。
5. OpenSpec change 只作为变更依据、验收和审计历史，不作为日常实现主入口。
6. 当测试暴露规范缺口时，先更新 owner 文档，再同步 schema、示例、实现和验证脚本。
7. 涉及共享 helper 的改动必须覆盖可观察外部行为：direct CLI warning placement、protocol-json stdout purity、readable warning/error 投影、adapter direct machine command boundary、adapter direct CLI 配置读取边界、Markdown pagination mechanics 和 schema/decode/semantic invalid paths。
