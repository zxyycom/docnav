...
配置场景使用 `test/smoke/core/fixtures/configs/` 下长期保留、按语义命名的 JSON fixture。CLI smoke 可以把 fixture 安装为临时 project 的 `.docnav/docnav.json` 来验证默认 project context，也可以把只读 fixture 直接通过 `--project-config` / `--user-config` 传给真实 CLI。会写入 project/user config 的场景必须先把 fixture 复制到临时文件，再把该临时文件作为 selected config path 传入，避免修改 checked-in fixture 或共享运行状态；文档路径直接指向共享 Markdown fixture。
...
- navigation-owned raw config source 读取、default missing absence、explicit missing/unreadable/invalid/non-object failure、navigation input resolution 来源合并、通用字段声明与 selected adapter declarations 注册、adapter option handoff 和 config failure 边界。
- operation 参数所有权。
