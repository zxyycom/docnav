...
配置场景使用 `test/smoke/core/fixtures/configs/` 下长期保留、按语义命名的 JSON fixture。CLI smoke 运行时把 fixture 安装为临时 project 的 `.docnav/docnav.json`，文档路径直接指向共享 Markdown fixture；会写入 project/user config 的场景只修改该临时副本，避免修改 checked-in fixture 或共享运行状态。
...
- core 配置 source descriptor/path 解析与 handoff、navigation-owned raw config source 读取、navigation input resolution 来源合并、通用字段声明与 selected adapter declarations 注册、adapter option handoff 和 config failure 边界。
- operation 参数所有权。
