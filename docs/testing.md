...
- static registry adapter source boundary、`adapter list` metadata inspection、dynamic management command removal 和 historical registration material ignored。
...
- core 配置路径解析、配置源读取、已注册字段映射、参数来源合并、源码级 native option registry/generic merge、adapter option handoff 和 config failure 边界。
- operation 参数所有权。
...
7. 涉及共享 helper 的改动必须覆盖可观察外部行为：core CLI strict failure placement、protocol-json stdout purity、primary readable/protocol error projection、protocol raw facts 到 readable display/cost/info projection、static registry adapter inspection、core 配置读取边界、Markdown pagination mechanics 和 schema/decode/semantic invalid paths。
