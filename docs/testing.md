...
7. 涉及共享 helper 的改动必须覆盖可观察外部行为：direct CLI warning placement、protocol-json stdout purity、readable warning/error 投影、protocol raw facts 到 readable display/cost/info projection、adapter direct machine command boundary、adapter direct CLI 配置读取边界、Markdown pagination mechanics 和 schema/decode/semantic invalid paths。
