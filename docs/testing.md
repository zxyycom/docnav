...
本文定义 Docnav 自动化测试的层级、所有权、统一验证入口和一致性审计规则。以下
材料分别维护语义测试账本、覆盖矩阵和发布包预验收：

- [语义测试 Case 维护](testing/case-maintenance.md)：测试变更时的 Case 粒度、当前
  实体闭合和验证流程。
- [Case topic 表](testing/cases/topics.json)：受控查询分区；
  `testing/cases/<topic>.md` 中的 Case 拥有稳定测试目的及当前实体映射。
...
测试首先是当前有效性证据：执行结果必须直接表明 owner 承诺的行为当前成立。
发现未来变化只是自动化验证的自然结果，不是证明目标；不得用“能够防止某种
...
测试意图或删除对应 Case，不得为了匹配账本文字增加防御性断言、测试专用探针，
也不得为了让测试能单独进入账本而机械拆分。project wrapper 从完整当前树发现
测试实体；每个实体必须关联至少一个能解释其存在价值的 Case。
...
- 当前测试实体的静态发现使用仓库精确锁定的 external ast-grep CLI，只能从
  `scripts/test-evidence/ast-grep.ts` 调用。它属于开发期验证依赖，不进入
  canonical release file set，也不改变产品期 `docnav-code` 直接链接 Rust crates、
  不得启动外部 executable 的边界。
...
profile 扫描完整当前源码，执行 Cargo、Bun 和 smoke 的 list 或 report，规范化
当前测试实体，并双向核对静态与 runtime 集合；随后校验 topic、Case、owner 以及
Case 与实体并集的闭合。任何漏项、悬空、重复、不支持形态、未知实体或缺少 Case
的实体都会阻断验证。Bun report 已执行 supported profile 中的完整 Bun 测试面，
因此 workspace verifier 不再重复调度 workspace、validator、release 或 quality
...
2. 测试函数变更已按[语义测试 Case 维护](testing/case-maintenance.md)完成静态、
