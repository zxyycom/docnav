...
- adapter direct CLI 配置映射、strict config source failure、help 参数展示和 `invoke` protocol stdin 与标准参数映射边界。
...
- adapter direct CLI 配置路径解析、配置源读取、已注册字段映射、参数来源合并、owner-scoped native option source 和 config failure 边界。
- operation 参数所有权。
...
- diagnostics stack/id/mark/order、DiagnosticCode details/primary `DiagnosticRecord` 投影、direct CLI argv strictness、document output orchestration、低层 JSON writer 和 paging helper 的可观察行为边界。
...
7. 涉及共享 helper 的改动必须覆盖可观察外部行为：direct CLI strict failure placement、protocol-json stdout purity、primary readable/protocol error projection、protocol raw facts 到 readable display/cost/info projection、adapter direct machine command boundary、adapter direct CLI 配置读取边界、Markdown pagination mechanics 和 schema/decode/semantic invalid paths。
