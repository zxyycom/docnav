...
| 单元 | parser、ref、默认值、分页、错误通道记录/projection 和其它自定义逻辑不变量 |
...
历史回归只作为风险线索或代表性输入来源，不作为证明目标。无论是新增 case、拆分 case，还是把断言嵌入已有 case，新增断言都必须先能写出“owner 明确承诺的语义 -> 可观察结果”的证明目标，并能追溯到当前 owner 文档、schema、示例、错误通道 projection 或覆盖矩阵。只有当明文契约要求校验缺失、拒绝、输出通道不污染、ref 不改变或其它否定性边界时，才测试该否定行为；否则使用现有覆盖、局部验证命令或代码审查证明本次改动。
...
- warning/error projection 的承载位置、stdout/stderr 边界和 schema 校验。
- adapter direct CLI 配置映射、配置源 warning、help 参数展示和 `invoke` protocol stdin 与标准参数映射边界。
- `invoke` 的 stdin 请求链路。
- 分页、continuation 和终止行为。
- core、adapter 和 release package 的跨进程链路。
...
- 参数 token 消费边界和兼容规则。
- adapter direct CLI 配置路径解析、配置源读取、已注册字段映射、参数来源合并和 native option 适用性边界。
- operation 参数所有权。
...
- diagnostics stack/id/mark/order、DiagnosticCode details/projection、direct CLI argv compatibility、document output orchestration、低层 JSON writer 和 paging helper 的可观察行为边界。
...
- `bun run typecheck:scripts` 证明脚本模块 contract 和边界类型一致，不替代真实 CLI、schema 或进程 smoke 验证。
...
- full profile 中的 smoke 审计日志分别写入 `.log/smoke/core/latest.log` 和 `.log/smoke/markdown/latest.log`。
...
| `bun run smoke:docnav` | 对当前开发构建运行 core 和 Markdown CLI smoke |
| `bun run cli:dev -- <args>` | 构建并运行当前开发版 `docnav` |
| `bun run cli:dev -- docnav-markdown <args>` | 构建并运行当前开发版 Markdown adapter |
| `bun --silent run dnm <args>` | 运行当前开发版 Markdown adapter，只保留命令结果和失败诊断 |
...
4. schema、示例和 fixture 只校验 protocol/readable 输出 projection，不成为新的业务语义或 code/details 规则来源。
5. OpenSpec change 只作为变更依据、验收和审计历史，不作为日常实现主入口。
6. 当测试暴露规范缺口时，先更新 owner 文档，再同步 schema、示例、实现和验证脚本。
7. 涉及共享 helper 的改动必须覆盖可观察外部行为：direct CLI warning placement、protocol-json stdout purity、readable warning/error projection、adapter direct machine command boundary、adapter direct CLI 配置读取边界、Markdown pagination mechanics 和 schema/decode/semantic invalid paths。
