...
```text
docnav outline <path> [--project-config <path>] [--user-config <path>] [--adapter <adapter-id>] [--pagination enabled|disabled] [--page 1] [--limit 6000] [--output readable-view|readable-json|protocol-json]
docnav read <path> --ref <ref> [--project-config <path>] [--user-config <path>] [--adapter <adapter-id>] [--pagination enabled|disabled] [--page 1] [--limit 6000] [--output readable-view|readable-json|protocol-json]
docnav find <path> --query <text> [--project-config <path>] [--user-config <path>] [--adapter <adapter-id>] [--pagination enabled|disabled] [--page 1] [--limit 6000] [--output readable-view|readable-json|protocol-json]
docnav info <path> [--project-config <path>] [--user-config <path>] [--adapter <adapter-id>] [--output readable-view|readable-json|protocol-json]
docnav init [--project-config <path>]
docnav doctor [--project-config <path>] [--user-config <path>]
docnav config get|set|unset|list [--user] [--path <path>] [--operation outline|read|find|info] [--project-config <path>] [--user-config <path>]
docnav adapter list
docnav version
```
...
- `docnav config get|set|unset|list` 支持 `--project-config <path>` 与 `--user-config <path>`。
...
- `docnav config set` 和 `unset` 默认写 selected project config file。
- `docnav config set --user` 和 `unset --user` 写 selected user config file；`--user` 只选择 scope，不选择文件路径。
- `config get` 的 key 不存在时返回 `INVALID_REQUEST`。
- `config list` 不带 path 时列出 core 配置域当前生效值。
- `config list --path <path> [--operation outline|read|find|info]` 使用 selected project/user config files 解析 document context，并展示该 path 触发的 adapter、参数来源和最终值。
