...
本文是 `docnav` 命令、`config` 命令入口、adapter 管理、adapter 直接 CLI、退出码和分页入口的主规范。输出模式的序列化、warning 承载和 readable rendering 见 [输出模式](output.md)；直接 CLI argv 映射、配置映射和标准参数机制见 [标准参数](standard-parameters.md)。
