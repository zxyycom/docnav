...
1. 项目根解析。
2. path 规范化和可访问性检查。
3. adapter 选择：显式 `--adapter` 或配置提供的 adapter id 按 declared intent 校验；未声明 adapter 时才进入 automatic discovery，并按 registry 候选探测返回第一个成功项。
...
未知 argv、多余 positional 和当前 operation 不使用的已知参数表达 invalid caller intent，必须在 document operation execution 和 adapter routing 前返回输入错误并写入错误通道。当前 operation 实际使用的参数同样严格校验：缺少必需 path/ref/query、非法 page、非法 limit、非法 output 或非法 native option 必须返回输入错误。当前 operation 不使用的 known 参数只需要按 token/operation membership 产生 applicability diagnostic，不触发其它 operation 的 eager typed validation。
...
Adapter 直接 CLI 默认值来自对应标准参数 definition 和 registration。`docnav-<adapter-id>` document operation 在构造 request 前，按标准参数机制解析显式 argv、adapter 配置、owner-scoped native option source 和内置默认值；request construction 只序列化 protocol 需要的显式字段，以及当前入口明确保留的 adapter-owned options。Adapter `invoke` 作为独立 protocol 入口重新处理 request。`docnav-markdown` 默认值见 [Markdown Adapter](adapters/markdown.md#默认值)。

Adapter direct CLI 配置路径发现、字段映射、来源合并和配置源失败规则由 [标准参数](standard-parameters.md#输入与配置映射) 定义。默认配置路径不存在表示 absent；显式 override 缺失、不可读、不是文件、invalid JSON、non-object JSON，或默认配置文件一旦存在但无效，都必须在 config input boundary 返回配置诊断。

`manifest`、`probe` 和 help 不读取 adapter direct CLI document operation 配置。Document operation help 必须展示 `--project-config-path <path>` 和 `--user-config-path <path>`；help 只输出参数说明，不执行配置读取或文档导航。`invoke` 不执行 adapter direct CLI argv parsing 或 help；stdin JSON 解码后的 protocol request `arguments` 是 `invoke` 的 direct input。`invoke` 的 request arguments、配置来源和默认值按标准参数机制映射、合并和校验；配置可以补足已注册的缺失参数，但不回写原始 stdin request JSON。后续交接见 [标准参数](standard-parameters.md#metadata-与交接边界)。
...
| Adapter direct document operations：`docnav-markdown outline/read/find/info` | adapter SDK + format adapter | 是 | 是；unknown、extra、unused known 和 undeclared native option 在执行前失败 | readable-view/readable-json/protocol-json 按输出模式投影 primary failure | `--output protocol-json` stdout 只含 protocol response envelope | 否 | adapter SDK 与格式 adapter 验收 |
| Adapter direct machine commands：`manifest/probe/invoke` | adapter SDK | `invoke` direct input 经标准参数解析后进入等价 semantic request；manifest/probe 否 | manifest/probe CLI argv 按 machine command owner 校验；invoke JSON 是 strict protocol direct input | manifest/probe 诊断写 stderr；invoke transport/validation error 写 protocol failure | stdout 只含 manifest、probe 或 protocol response schema payload | 否 | adapter contract 验收 |
