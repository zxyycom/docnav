...
1. 信任仓库的 mise 配置。
2. 按 lockfile 安装工具和根包依赖。
3. 预取根 Cargo workspace 的锁定依赖。
4. 初始化或同步 CodeGraph 索引。
...
1. 锁定工具和本地包命令可用。
2. 根 Cargo workspace 可按 lockfile 离线解析，且包含所有仓库内 Rust crate。
3. CodeGraph 索引存在且与工作区同步。
...
[语义测试 Case 维护](testing/case-maintenance.md)拥有稳定的账本与闭合规则，
`docs/testing/cases/` 拥有 topic 与当前 Case 内容。仓库跟踪的
...
2. 项目环境入口需要在包依赖安装前运行，因此由根仓库脚本直接持有同一组 child env。
