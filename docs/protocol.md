...
错误 message 和 guidance 是可定制文案；调用方只解析 code 和 details。`INVALID_REQUEST` 可以在 details 中附带 `path`、`received`、`expected` 或 `accepted` 等定位和校正信息；这些补充字段不得替代必需的 `field` 和 `reason`。
