...
- 共享协议、`docnav` core、MCP 和其它接入层只校验 ref 是非空字符串，并把收到的值原样传给选定 adapter。
...
| 层 | 正确性责任 |
| --- | --- |
| 共享层（`docnav` core、MCP、协议、schema） | 按 path 选择正确 adapter；保持 ref 原值不变；一致映射稳定错误 |
...
共享层不要求每个 adapter 必须产生上述全部错误。每个 adapter 在其专属文档中说明实际可能返回的错误及其边界。`REF_AMBIGUOUS` 保留为可用稳定错误；Markdown adapter 当前解析结果中的 canonical ref 具有唯一性，因此不会产生该错误。

## 格式定位所有权

共享协议、`docnav` 和接入层不得解析、拼接、规范化或从 display 推断 ref，只能把 adapter 返回的 ref 原样传递给 read。adapter 可以在没有局部导航区域时为文档定义私有全文 ref；该 ref 的具体拼写和语义属于对应 adapter。
