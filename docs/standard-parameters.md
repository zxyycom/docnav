...
Native option registry view 只用于源码级 owner/namespace/type variant 发现、CLI/config/protocol source classification、metadata 展示和合并 handoff。`options.max_heading_level` 这类格式语义仍属于 consuming adapter；core 只按 selected adapter descriptor 投影支持的 entries 并报告 unsupported option，Adapter 在消费最终 option values 时负责返回 type mismatch 或 range invalid 等 adapter-owned structured diagnostic。
