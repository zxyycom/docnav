...
标准参数机制定义 CLI、配置文件和 protocol request `arguments` 如何表达同一组导航参数。它的核心行为是：把不同入口的输入映射到同一个标准参数对象，按固定来源顺序合并，完成校验，然后把安全结果交给后续 owner 消费。
...
```mermaid
flowchart TD
  A["Definition<br/>LIMIT<br/>identity / schema / static default / dynamic default"] --> B["Registration<br/>CLI flag / config path / protocol argument"]

  C["Direct input<br/>CLI argv / protocol arguments"] --> D["映射为 source object<br/>source=direct<br/>limit + native option sources"]
  D --> E["解析项目与用户上下文<br/>project root / user config owner"]
  E --> F["确定 core 配置源<br/>.docnav/docnav.json + user docnav.json"]
  ...
  C --> N["unmapped/native-option processing result"]
  G --> N
  N --> O["按入口策略交接<br/>diagnostic / delegated native option source"]
  O --> M

  M --> P["后续流程消费<br/>typed values / source info / diagnostics / native option sources"]

  A -. "注册冲突或默认值非法" .-> X1["build/register failure"]
  G -. "present config invalid 或 required source unavailable" .-> X2["blocking config-source 诊断<br/>入口返回 config failure"]
  C -. "未知 argv / extra positional / unused flag / unmapped field" .-> X3["source-scoped unmapped-input 诊断<br/>入口返回 input failure"]
...
| Config source | 默认项目或用户配置源缺失表示来源不存在；已存在的配置源不可读、不是文件、JSON 无效或顶层不是 object 时，产生 blocking config-source 诊断交接数据，入口返回配置失败。 |
| Direct input shape | CLI argv tokens 和已解码的 protocol JSON value 直接进入 direct processing paths；缺字段、类型错误、requiredness 和 constraint 失败由标准参数/typed-field validation 产生诊断交接数据，入口 owner 再映射为输入错误或 `INVALID_REQUEST`。Malformed JSON 属于 protocol transport decode failure，不能形成 direct input。 |
| Direct CLI strictness | 未知 argv、多余 positional、当前 operation 不使用的已知 flag 或无法归入源码级 native option registry/source 的输入生成 source-scoped unmapped-input 或 operation-applicability 诊断交接数据；入口 owner 在 document operation execution 前返回输入失败。 |
| Standard validation | 类型、范围、枚举、requiredness 或 default 结果不满足 schema facet 时返回 validation 诊断交接数据；CLI 映射为输入错误，protocol request 映射为 `INVALID_REQUEST`。 |
...
配置读取先校验 JSON 顶层 object，再按已注册的配置路径映射字段。未知顶层字段、未知 `defaults` 字段、无法归入源码级 native option registry 的 `options` key 或其它未映射配置字段产生 config input diagnostic；不会被静默跳过。已归入 registry 的 native option raw value 会保留到最终 option source，由 adapter 在消费时判断 unsupported option、type mismatch 或 range invalid。Document identity 参数不从配置域读取：`path`、`ref` 和 `query` 只能来自显式输入；`page` 来自显式输入或入口固定默认 `1`，不使用 `defaults.page` 一类配置默认值。
...
标准参数 pipeline 不解释 native option 的格式语义，不按 selected adapter 过滤，也不把同名 option 合并为单一类型。需要 raw-minus-mapped 或 native option source 时，入口处理函数可以返回定位器、变更描述或已经组织好的剩余 JSON 子树。default 没有原始输入对象，因此没有未映射输入处理结果。
...
当标准参数需要跨 protocol 传给 adapter 时，operation argument binding 仅用于把标准参数 identity 映射到 request `arguments` path。跨 protocol 序列化发生在当前入口完成标准参数解析之后；序列化时只写入需要传递的 direct input 字段，以及入口 owner 从 native option source 中明确选择保留的字段。已解析出的配置值、用户配置值或默认值不会仅因 request construction 被重新分类为 protocol request direct input。

Native option registry view 只用于源码级 owner/namespace/type variant 发现、source classification、metadata 展示和合并 handoff。`options.max_heading_level` 这类格式语义仍属于 consuming adapter；Adapter 在消费最终 option values 时负责返回 adapter-owned structured diagnostic，例如 unsupported option、type mismatch 或 range invalid。
...
- `limit` 的 identity、canonical key、CLI spelling、config path 和 protocol argument path 必须保持同一参数；预算单位和分页语义不得在标准参数层重新定义。
...
- 非 owner 文档只保留消费边界摘要和本文链接，不复制标准参数的来源、合并、native option registry/handoff 或校验规则。
