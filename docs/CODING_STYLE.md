...
具体命令、协议字段、adapter 契约、ref 语义、schema 形状和测试矩阵，以 `README.md` 的“如何阅读这些文档”指向的主规范为准。编码规范只回答一个问题：实现这些契约时，代码应该保持什么性质。
...
做法：把失败转换为对应稳定错误、验证失败或诊断信息，并保留足够的定位依据。
