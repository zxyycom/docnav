...
- `docnav-typed-fields`：字段级事实源 owner，承接 field identity、structured JSON path、value kind、字段级 constraints、static default metadata、decode/validation attribution、schema metadata view 和 duplicate identity guard；输出 typed values object 与字段事实 projection，供标准参数、JSON contract validation 或 schema tooling 消费。来源合并、CLI argv parsing、operation binding、manifest/probe policy、protocol envelope、readable output 和完整 JSON Schema document generation 仍由对应 consumer owner 定义。
- 标准参数共享层：规则见 [标准参数](standard-parameters.md)。该层承接参数 identity、入口字段映射、配置字段映射、来源标记、合并顺序、默认值、透传策略、validation 和 operation binding；core、SDK 和 adapter `invoke` 只消费对应声明与解析结果。
