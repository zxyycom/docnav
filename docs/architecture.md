...
- `docnav-typed-fields`：字段级事实源 owner，承接 field identity、extraction strategy declaration、strategy input kind guard、JSON path strategy 的 structured path、processing build、value kind、字段级 constraints、static default metadata、decode/validation attribution、schema metadata view 和 duplicate identity guard；输出 typed values object、caller processing result 与字段事实 projection，供标准参数、JSON contract validation 或 schema tooling 消费。来源合并、CLI argv parsing、operation binding、manifest/probe policy、protocol envelope、readable output 和完整 JSON Schema document generation 仍由对应 consumer owner 定义。
