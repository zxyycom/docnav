...
- `docnav-typed-fields`：字段级事实源 owner，承接 field identity、processing strategy declaration、processing input kind guard、processing build、value kind、字段级 constraints、static default metadata、validation attribution、schema metadata view 和 duplicate identity guard。`FieldDefSet` 聚合 typed field definitions，并提供 metadata、processing-path lookup、input-kind guard 和 candidate value validation 所需事实；input-specific helper 负责把具体输入格式映射到 `FieldDefSet` 的 metadata/validation。当前 JSON helper 承接 JSON path structured path、`serde_json::Value` extraction、unknown-field detection 和 caller processing result。来源合并、CLI argv parsing、operation binding、manifest/probe policy、protocol envelope、readable output、adapter-id namespace policy、native option handoff policy 和完整 JSON Schema document generation 仍由对应 consumer owner 定义。
- `cli-config-resolution`：framework-independent CLI/config resolution core owner，提供 field contract、ordered source collection、source candidate、field-level merge、provenance trace、resolution diagnostics 和 typed materialization。应用 command structure、config layout、public diagnostics、adapter/operation/protocol/output 语义仍由 consumer owner 定义。
- `cli-config-resolution-clap` 与 `cli-config-resolution-serde`：framework companion owner；前者把 CLI field projection 映射为 `clap` arguments 和 source candidates，后者把 `serde_json::Value` 中已声明的 config paths 映射为 source candidates。resolution policy 和应用 config layout 不下沉到 companion crates。

Parameter aggregation 是上述 existing pieces 的边界关系，而不是新的长期 owner。Core CLI flag owner-map 负责 public argv surface；`AdapterOptionSpec` 负责 adapter-owned native option facts；`docnav-typed-fields` 负责 Docnav 字段声明和 validation metadata；`cli-config-resolution` 负责把通用 field contracts 与 source candidates 机械组合为 resolution result；`docnav-navigation` 负责 Docnav metadata/source mapping、diagnostic projection 和 request construction。CLI/input projection 与 config-source projection 必须复用这些 facts，不能在 core config command 中维护第二套 output enum、positive integer、adapter option range/default 或 outline selector 语义。
