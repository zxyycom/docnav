...
- `cli-config-resolution-clap` 与 `cli-config-resolution-serde`：canonical `FieldDefSet` 的 input companion。前者从 CLI flag processing metadata 注册 `clap` arguments 并提取 CLI source candidates；后者只从 `serde_json::Value` 提取已声明 config paths 的 source candidates。未知 CLI argument 由 `clap` 正常拒绝；应用是否严格拒绝额外 config 字段、字段 applicability 和 public diagnostic mapping 由 consumer owner 决定。
