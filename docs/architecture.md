...
- `docnav-parameter-resolution`：当前实现中的参数来源解析 helper。`ParameterResolutionPipeline` 消费 `docnav-typed-fields` metadata/validation、navigation common field declarations 和 use-site registered `AdapterOptionSpec` declarations，提供 source kind/source info、来源合并、默认值、diagnostic handoff 和 operation argument binding metadata。它可以承接 parameter aggregation 的机械 projection，但不是 parallel aggregation layer，也不拥有字段语义：CLI flag owner-map、config-source path、adapter option value kind/range/default 和 selected adapter namespace 都来自对应 owner metadata。长期产品入口按 [Navigation Input Resolution](navigation-input-resolution.md) 描述，`docnav-navigation` 负责加载 navigation config sources、合并通用字段和 selected adapter 注册字段、解析来源并构造 request。

Parameter aggregation 是上述 existing pieces 的边界关系，而不是新的长期 owner。Core CLI flag owner-map 负责 public argv surface；`AdapterOptionSpec` 负责 adapter-owned native option facts；`FieldDefSet` 负责字段事实和 validation metadata；`ParameterResolutionPipeline` 负责把来源和字段 facts 机械组合为解析结果。CLI/input projection 与 config-source projection 必须复用这些 facts，不能在 core config command 中维护第二套 output enum、positive integer、adapter option range/default 或 outline selector 语义。
