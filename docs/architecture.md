...
- 标准参数共享层：规则见 [标准参数](standard-parameters.md)。该层承接参数 identity、入口字段映射、配置字段映射、来源标记、合并顺序、默认值、透传策略、validation、operation binding 和 MCP metadata；core、SDK、adapter `invoke` 和 MCP bridge 只消费对应声明与解析结果。
...
- 配置不得改变 protocol envelope、readable JSON 字段、MCP structuredContent 字段或稳定错误 code；这些字段由对应 owner 文档定义。
