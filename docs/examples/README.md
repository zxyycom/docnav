...
- MCP bridge 目标 structuredContent 返回精简 readable 结果，不包含 envelope；直接 CLI warning 行为遵循 [CLI](../cli.md#直接-cli-兼容参数规则)，warning item 使用 `id`、非空 `reason`、稳定 `effect` 和 `details` 对象。
...
示例只证明 protocol/readable、manifest 和 probe 的 documented shape。direct CLI warning placement、protocol-json stdout purity、adapter machine command 边界和 pagination mechanics 由主规范、smoke 和 Rust 回归测试验收。
