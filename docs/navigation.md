...
| readable-view | 文档操作的默认阅读输出；完整格式、block 字段、byte framing 和 warning/error 规则由 [CLI 与 MCP 输出](cli.md#readable-view) 定义。 |
...
| renderer config | 仓库内提交的 readable-view block 字段声明；用户配置不得修改；完整规则见 [CLI 与 MCP 输出](cli.md#readable-view)。 |
