...
| `json-adapter` | [JSON Adapter](adapters/json.md) | JSON probe/private model/ref/outline/read/find/info/full-read 和 JSON-owned errors |
...
| 载体 | 拥有 | 不拥有 |
| --- | --- | --- |
| `docs/` owner 文档 | 稳定行为、public contract、职责边界和验证语义 | 当前支持证明、change 任务或决策演进历史 |
| `openspec/changes/<change>/` | 当前 change 的目标、设计、`## Decisions`、任务、验收依据和审计历史 | 稳定规则的最终 owner 或当前实现状态 |
| `docs/decisions/` | 已确认且跨 change 仍有效的目的、背景、采用方向和生命周期关系 | 当前行为、实现状态、任务清单或完整 change 设计 |

按以下顺序记录和同步：

1. 只影响某个 active change 的已确认判断写入该 change 的 `design.md` `## Decisions`，不重复写入全局决策集合。
...
| JSON probe/parse、private model、ref grammar、导航顺序、source-region find、structured/full-read 和 JSON-owned error 边界 | [JSON Adapter](adapters/json.md) |
