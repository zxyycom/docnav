...
| JSON adapter 与 pathname routing | Adapter crate 覆盖完整有序 manifest hints、selected-operation parse、单一 primary model、ref、source-order traversal、raw-number content、source-occurrence find、info/full-read、Unicode pagination/cost 和四种 exact `DOCUMENT_CONTENT_INVALID` reason；core 逐一覆盖 expanded JSON complete-basename hints，并验证 static registry、route-before-I/O、closed public input、automatic/explicit selection、no-fallback 与 private routing state；真实 CLI 覆盖 exact descriptor、代表性 suffix/exact-filename `outline -> ref -> read`、raw/readable、exact pathname no-match 和代表性 ref/document failure。 | Rust JSON adapter/core tests、core CLI smoke、package smoke |
...
| release package | 覆盖 core-only package manifest、文件集合、校验和、host/target 选择，以及 manifest 指向的非符号链接普通 core binary；package smoke 必须从该 manifest entry 解析同一个 `docnav`，完成 required adapter inspection、Markdown roundtrip，以及 JSON representative suffix/exact-filename direct roundtrip。 | release package scripts、package smoke |
