...
| adapter inspection | descriptor metadata、support check 和 `adapter list` 覆盖 static registry metadata、linked handler availability 和 adapter layer 可用性；manifest/probe-shaped JSON 只作为 schema/example contract material。 | CLI smoke、schema/docs validators、Rust core/adapter tests |
| adapter source boundary | 默认 adapter implementation source 是 core release static registry 中的 linked adapter libraries；historical registration material、external executable 和 command path 不参与 document operation fallback。 | Core CLI smoke、Rust core tests |
| ref 与分页 | 至少覆盖 `outline -> ref -> read`、`find -> ref -> read`、invalid/not-found ref、分页继续和终止。 | CLI smoke、Rust adapter tests |
| 错误通道与投影阶段 | 覆盖 CLI 输入错误、adapter selection explicit failure、missing adapter + invalid-looking option 时 selection diagnostic 优先、automatic discovery all-failed candidate list、selected adapter layer failure、adapter-owned option validation failure、ref error、primary `DiagnosticRecord` protocol/readable 投影、canonical details 和 stack id/mark/LIFO 语义的代表样本。 | CLI smoke、schema/docs validators、Rust diagnostics/output tests |
| 配置与 path context | 覆盖 user/project/default 合并、`--path` context、core 配置路径覆盖、protocol request arguments 到标准参数的映射、source-level static native option registry 暴露和同名多 owner/type variant model、default config absence、explicit/present invalid config failure、config key/source/shape failure、adapter-side option type/range validation、help 不读取配置和配置不回写原始 protocol JSON 的边界。 | Core CLI smoke、Rust config tests |
