...
| 错误通道与投影阶段 | 覆盖 CLI 输入错误、adapter selection explicit failure、automatic discovery all-failed candidate list、selected invoke failure、ref error、primary `DiagnosticRecord` protocol/readable 投影、canonical details 和 stack id/mark/LIFO 语义的代表样本。 | CLI smoke、schema/docs validators、Rust diagnostics/output tests |
| 配置与 path context | 覆盖 user/project/default 合并、`--path` context、adapter direct CLI 配置路径覆盖、adapter `invoke` request arguments 到标准参数的映射、invoke 配置/default 补足、default config absence、explicit/present invalid config failure、非法配置值、help 不读取配置和配置不回写原始 protocol JSON 的边界。 | Core/Markdown CLI smoke、Rust config tests |
| release package | 覆盖 manifest、文件集合、校验和、host/target 选择和 package 内二进制 smoke。 | release package scripts、package smoke |

## 层级选择

- CLI smoke：证明真实进程入口、跨二进制链路、stdout/stderr、exit code、strict failure/error 投影承载位置和 package 可执行性。
