...
责任域当前使用 `CORE`、`MD`、`PROTO`、`READABLE`、`SDK`、`DIAG`、`CLIARGS`、`JSONIO`、`OUTPUT`、`STDPARAMS`、`WORKSPACE`、`SMOKE`、`PARALLEL`、`QUALITY`、`RELEASE`、`CASE`。新增责任域时，先在本文登记责任域，再同步账本条目和源码 `@case` 标记。
...
7. 新增、拆分或扩展测试断言前，先写出“owner 明确承诺的语义 -> 可观察结果”的证明目标；涉及错误通道时，证明目标必须区分 stack/code/details 规则、surface projection、schema/example/fixture 校验和 consumer 行为。历史回归、默认不存在或“没有发生某事”只能作为输入选择或风险说明，不能替代证明目标，除非 owner 文档、schema、示例或覆盖矩阵明文要求校验该否定边界。
...
否定性断言只在 owner 文档把该行为定义为稳定契约、安全边界、诊断 projection 或输出通道边界时进入测试。该限制同时适用于新增 case、拆分 case、为已有 case 增加分支，以及在已有正向行为链路中增加辅助断言。
